#!/usr/bin/env bash
# SC-008 production migration — applies 20260728160000_add_site_information.
#
...
#
# The migration is purely additive (one new table, SiteInformation, 1:1 with
# JobSite; all columns nullable). No data backfill; backwards-compatible with the
# running SC-007 code; safe to apply BEFORE the code deploy, no downtime.
set -euo pipefail
...
echo "== SC-008 PRODUCTION MIGRATION =="
...
(async () => {
  const tbl = await p.$queryRawUnsafe(`
    ...
    WHERE table_schema = '"'"'public'"'"'
      AND table_name IN ('"'"'SiteInformation'"'"')`);
  const cols = await p.$queryRawUnsafe(`
    SELECT column_name FROM information_schema.columns
    WHERE table_schema = '"'"'public'"'"' AND table_name = '"'"'SiteInformation'"'"'
      AND column_name IN ('"'"'workingHours'"'"','"'"'siteRules'"'"','"'"'welfareFacilities'"'"','"'"'siteHazards'"'"','"'"'emergencyProcedures'"'"','"'"'siteMapBlobPath'"'"')`);
  console.log("      table:", tbl.map(r => r.table_name).join(", ") || "(none)");
  console.log("      columns:", cols.length, "of 6");
  const ok = tbl.length === 1 && cols.length === 6;
...
echo "== SC-008 MIGRATION COMPLETE =="
