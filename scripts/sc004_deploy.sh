#!/usr/bin/env bash
# SC-004 production CODE deploy (commit a873ce6). No database migration — the
# data model already supports multiple concurrent open check-ins per worker, so
# SC-004 is code-only. Same proven flow as the SC-003 / OTP-fix deploys.
set -uo pipefail
...
echo "== SC-004 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-004 code IS present in this artifact..."
[ -f app/worker/page.tsx ] \
  && [ -f app/api/worker/active-site/route.ts ] \
  && [ -f components/worker/SiteSwitcher.tsx ] \
  && grep -q 'getActiveWorkerSiteId' services/workerDashboard/workerDashboardService.ts \
  && echo "      confirmed: worker home + site switcher + multi-site context present." \
  || { echo "ERROR: SC-004 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-004 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
