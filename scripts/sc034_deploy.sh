#!/usr/bin/env bash
# SC-034 — production CODE deploy: Integrations tabbed workspace.
#
...
APP=sitecomply-web
PG=sitecomply-pg
PRECHECK_RULE=tmp-sc034-precheck
SCM="https://${APP}.scm.azurewebsites.net"
BASE="https://${APP}.azurewebsites.net"
HEALTH="${BASE}/api/health"
ZIP=/tmp/sc034_deploy.zip
...
echo "== SC-034 — INTEGRATIONS TABBED WORKSPACE =="
...
if [ "$PRESENT" != "2" ]; then
  ...
  echo "       RUN scripts/sc034_migrate.sh FIRST."
  exit 1
fi
...
OUTSIDE=$(git diff --name-only "${BASE_COMMIT}"..HEAD \
  ...
  | grep -v '^scripts/' || true)
if [ -n "$OUTSIDE" ]; then
  echo "ERROR: SC-034 is a presentation change to the Integrations page, but these also changed:"
  echo "$OUTSIDE" | sed 's/^/         /'
  exit 1
fi
...
// ---------- SC-034 Integrations tabs ----------
// NOTE: no double quotes and no backticks in comments inside this node -e
// block — either one breaks out of the shell string and silently truncates.
const itPage=strip('app/admin/(dashboard)/settings/integrations/page.tsx');
const itTabs=strip('components/admin/AdminTabs.tsx');

// ---- the three tabs must all be reachable ----
for(const k of ['sms','cscs','ai']){
  if(itPage.indexOf(k + String.fromCharCode(39)) < 0 && itPage.indexOf(String.fromCharCode(39) + k) < 0)
    fail('the ' + k + ' tab is missing from the Integrations workspace');
}

// ---- EVERY component must still be rendered, with its data ----
// This is the assertion that matters: a tabbed rewrite is exactly the change
// that quietly drops a panel nobody notices until they go looking for it.
for(const c of ['SmsProviderSettings','SmsActivityLog','CscsProviderSettings','AiProviderSettings']){
  if(itPage.indexOf(c) < 0) fail(c + ' is no longer rendered — a tabbed rewrite must not drop a panel');
}
for(const prop of ['smsConfig','aiConfig','cscsConfig','smsLog','smsUsage']){
  if(itPage.indexOf(prop) < 0) fail(prop + ' is no longer loaded — a panel would render without its data');
}
// The read-only path for VIEWER admins must survive.
if(itPage.indexOf('ReadOnlyBanner') < 0) fail('the read-only banner for VIEWER admins is gone');
if(itPage.indexOf('adminCanManage') < 0) fail('the write gate is gone from the Integrations page');

// ---- an unknown tab must not render an empty workspace ----
if(itPage.indexOf('TABS.some') < 0)
  fail('an unknown tab is no longer validated — a stale bookmark would render nothing');

// ---- the CSCS heading must not be duplicated ----
// CscsProviderSettings renders its own h2. A page-level heading above it
// printed the name twice, which is what the first cut of this change did.
const cscsBlock=itPage.slice(itPage.indexOf('active === ' + String.fromCharCode(39) + 'cscs'));
if(cscsBlock.slice(0, 400).indexOf('CSCS Smart Check</h2>') >= 0)
  fail('the CSCS tab has a page-level heading again — the component renders its own, so it prints twice');

// ---- the tab must stay an address, not local state ----
if(itTabs.indexOf('use client') >= 0)
  fail('AdminTabs became a client component — the page already knows the active tab');
if(itTabs.indexOf('aria-current') < 0)
  fail('the tab strip no longer marks the current tab for assistive technology');

console.log('      confirmed: three tabs reachable, all four settings panels');
console.log('                 still rendered with their data, the read-only gate');
console.log('                 intact, unknown tabs validated, no duplicated CSCS');
console.log('                 heading, and the strip is still server-rendered.');
...
for path in \
  ...
  # pattern. SC-034 printed "routes: all reachable" while every route was
...
if [ "$CODE" = "200" ] && [ -z "$SMOKE_FAIL" ]; then
  echo "== SC-034 DEPLOYED =="
  echo "   Presentation only. Every integration setting behaves exactly as before."
else
