...
ZIP=/tmp/reportattribution_deploy.zip
DEPLOYED=9cd05fc
...
echo "== REPORT ATTRIBUTION DEPLOY =="
...
scripts/deploy/reportattribution_deploy.sh
...
# --- The client sends it, and it cannot be shadowed.
code components/ui/ReportIssueDialog.tsx | grep -q '\.\.\.context, portal }' \
  || die "portal is not spread last in the request body"
code components/ui/ReportIssueButton.tsx | grep -q 'portal={portal}' || die "the button does not pass its portal"
for f in "components/worker/WorkerShell.tsx:WORKER" "components/platform/PlatformShell.tsx:PLATFORM" "components/admin/AdminShell.tsx:ADMIN"; do
  file="${f%%:*}"; want="${f##*:}"
  grep -q "portal=\"$want\"" "$file" || die "$file no longer declares portal=\"$want\""
done
echo "      each shell declares its portal and the dialog sends it last."

# --- A stale bundle must still work.
code "$SVC" | grep -q 'return (await platformReporter()) ?? (await adminReporter()) ?? (await workerReporter());' \
  || die "the no-portal fallback is gone; cached bundles would break"
echo "      a client that sends no portal still files reports."
