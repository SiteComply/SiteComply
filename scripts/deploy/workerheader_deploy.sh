#!/usr/bin/env bash
# Deploy: worker header — unique sites in the switcher, no Check out overlap.
#
...
ZIP=/tmp/workerheader_deploy.zip
...
scripts/deploy/workerheader_deploy.sh
scripts/workerheader_verify.js
scripts/workerheader_workflow_verify.js
services/workerDashboard/workerDashboardService.ts"
CH=$(git diff --name-only "$DEPLOYED" HEAD | sort)
[ "$CH" = "$(printf '%s' "$EXPECTED" | sort)" ] \
  && echo "      confirmed: the two header components, the service and scripts." \
...
code "$SVC" | grep -qF 'findIndex((o) => o.jobSiteId === s.jobSiteId) === i' \
  || { echo "ERROR: openCheckIns is not deduped by site. Aborting"; exit 1; }
echo "      S1: openCheckIns deduped by jobSiteId."

# S2 — the site control must be able to shrink, and must no longer be fixed.
code "$SW" | grep -qF 'w-full min-w-0' \
  || { echo "ERROR: the select cannot shrink — the overlap would return. Aborting"; exit 1; }
code "$SW" | grep -qF 'max-w-[12rem]' \
  && { echo "ERROR: the fixed 12rem width is back. Aborting"; exit 1; }
...
grep -rqF 'w-full min-w-0 truncate' .next/server 2>/dev/null \
  && echo "      the flexible switcher classes compiled in." \
  || { echo "ERROR: switcher classes absent from the bundle. Aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== WORKER HEADER DEPLOY COMPLETE ==" || { echo "== HEALTH NOT 200 =="; exit 3; }
