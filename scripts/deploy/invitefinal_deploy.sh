...
ZIP=/tmp/invitefinal_deploy.zip
...
echo "== INVITE WORKER FINAL STYLING DEPLOY =="
...
# Solid green at LISTING dimensions.
grep -q 'bg-safe-500 px-3 py-1.5 text-sm font-semibold text-white' "$WK" \
  && echo "      confirmed: solid green at px-3 py-1.5." \
  || { echo "ERROR: expected compact solid-green styling — aborting"; exit 1; }

# The invite Link must NOT carry touch-target — that min-h/min-w 3.25rem is
# what pinned both previous versions at 52px regardless of padding.
if grep -A3 'href={inviteHref}' "$WK" | grep -q 'touch-target'; then
  echo "ERROR: touch-target back on the invite button — it overrides the height. Aborting"; exit 1
fi
echo "      confirmed: no touch-target on the invite button."

# The toolbar must be the table's own control strip, or it floats.
grep -q 'TABLE_TOOLBAR_CLASS' "$WK" \
  && grep -q "'w-full items-center justify-between'" "$WK" \
  && echo "      confirmed: toolbar attached + vertically centred." \
  || { echo "ERROR: toolbar not using TABLE_TOOLBAR_CLASS — aborting"; exit 1; }

# Placement, behaviour and the single invite path are untouched by styling.
...
grep -rqF "bg-safe-500 px-3 py-1.5" .next/server 2>/dev/null \
  || { echo "ERROR: compact green button not in bundle — aborting"; exit 1; }
...
echo "      confirmed: compact green trigger in bundle; page intact."
...
[ "$CODE" = "200" ] && echo "== INVITE WORKER FINAL STYLING COMPLETE ==" || { echo "== HEALTH NOT 200 — investigate =="; exit 3; }
