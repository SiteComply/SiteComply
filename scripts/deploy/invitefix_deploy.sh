...
echo "== INVITE WORKER CLICK FIX DEPLOY =="
...
# THE REGRESSION THAT CAUSED THIS: useState(prop) is mount-only, so the dialog
# must be synced by an effect or a client-side click does nothing at all.
grep -qE 'useEffect\(\(\) => \{\s*$' "$WM" && grep -q 'if (autoOpenInvite) setShowInvite(true);' "$WM" \
  && echo "      confirmed: autoOpenInvite synced by effect, not just useState." \
  || { echo "ERROR: no effect syncing autoOpenInvite — a click will do nothing. Aborting"; exit 1; }

grep -q "next.delete('invite')" "$WM" \
  && echo "      confirmed: ?invite=1 cleared on close (repeat clicks work)." \
  || { echo "ERROR: invite param not cleared on close — button works once only. Aborting"; exit 1; }
...
# The fix must reach the CLIENT chunk — server-side presence proves nothing,
# because the failing path is a client-side transition.
...
grep -q 'useEffect)(()=>{f&&' "$CHUNK" 2>/dev/null \
  || { echo "ERROR: prop-sync effect absent from the client chunk — the click will do nothing. Aborting"; exit 1; }
grep -q '"invite"' "$CHUNK" 2>/dev/null \
  || { echo "ERROR: invite param handling absent from client chunk — aborting"; exit 1; }
echo "      confirmed: prop-sync effect present in the CLIENT chunk."
...
[ "$CODE" = "200" ] && echo "== INVITE WORKER CLICK FIX COMPLETE ==" || { echo "== HEALTH NOT 200 — investigate =="; exit 3; }
