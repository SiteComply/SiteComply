#!/usr/bin/env bash
# Deploy: filter default options stop repeating the field label.
#
...
ZIP=/tmp/filterwording_deploy.zip
DEPLOYED=7299a69
LABELLED="app/platform/dashboard/documents/page.tsx
...
[ "$N" = "8" ] && echo "      eight labelled defaults read \"All\"." \
  || { echo "ERROR: found $N labelled \"All\" defaults, expected 8. Aborting"; exit 1; }

# The old wording must be gone from those four files...
for old in 'All categories' 'All statuses' 'All priorities' 'All my sites'; do
...
[ "$CODE" = "200" ] && echo "== FILTER WORDING DEPLOY COMPLETE ==" || { echo "== HEALTH NOT 200 =="; exit 3; }
