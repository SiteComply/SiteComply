#!/usr/bin/env bash
# SC-048 — production CODE deploy: Reports catalogue layout.
#
# The Reports landing becomes a directory of rows instead of a 3-column grid of
# equal-weight cards. Presentation only — same registry, same access filter,
# same routes.
#
...
echo "== SC-048 — REPORTS CATALOGUE LAYOUT =="
...
if [ -n "$TOUCHED" ]; then
  ...
  echo "$TOUCHED" | sed 's/^/         /'
  echo "       SC-048 is a colour change. If the devCode suppression has been"
...
OUTSIDE=$(git diff --name-only "${BASE_COMMIT}"..HEAD \
  ...
  | grep -v '^services/reports/reportsLayout.ts$' \
  | grep -v '^scripts/' || true)
if [ -n "$OUTSIDE" ]; then
  echo "ERROR: SC-048 fixes ONE defect, but these also changed:"
  echo "$OUTSIDE" | sed 's/^/         /'
  exit 1
fi
...
npx tsx scripts/sc001_cscs_tests.ts > /tmp/sc048_cscs.log 2>&1
if [ $? -ne 0 ]; then
  echo "ERROR: the CSCS verification suite failed."
  tail -15 /tmp/sc048_cscs.log | sed 's/^/         /'
  exit 1
fi
echo "      $(tail -2 /tmp/sc048_cscs.log | tr -d '\n' | sed 's/.*== //')"

echo "[4/7] Building and packaging..."
npx prisma generate > /dev/null 2>&1 || { echo "ERROR: prisma generate failed"; exit 1; }
npm run build > /tmp/sc048_build.log 2>&1
if [ $? -ne 0 ]; then
  echo "ERROR: build failed. Last lines:"
  tail -25 /tmp/sc048_build.log | sed 's/^/         /'
  exit 1
fi
grep -qE "Compiled successfully" /tmp/sc048_build.log || { echo "ERROR: build did not report success"; exit 1; }
