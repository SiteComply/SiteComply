#!/usr/bin/env bash
# THE LIBRARY'S INFORMATION ARCHITECTURE.
#
...
grep -qF "not available yet" "$INDEX" \
  || fail "the generated option no longer says it is unavailable - it would silently fail"
...
if [ -z "$DRY" ]; then
  ...
  COLS=$(PGOPTIONS='-c default_transaction_read_only=on' psql "$DB" -X -tA -c "SELECT count(*) FROM information_schema.columns WHERE (table_name='LibraryAsset' AND column_name IN ('category','provenance')) OR (table_name='LibraryAssetRevision' AND column_name='sourceModuleRevisionId')" 2>/dev/null)
  [ "$COLS" = "3" ] || fail "production has $COLS of the 3 new columns - run ~/library_ia_migrate.sh first"
...
for S in "Where will the video come from?" "Reusable company footage that every project" \
         "Where this video is used" "Company standards"; do
