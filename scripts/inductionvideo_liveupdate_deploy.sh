#!/usr/bin/env bash
# INDUCTION VIDEO SCREENS KEEP THEMSELVES CURRENT WHILE A JOB RUNS.
#
# NO MIGRATION. A UI change: a shared client component refreshes the page while a
# generation, narration or render job is in flight, and stops when it finishes.
#
# WHAT MUST BE TRUE AFTERWARDS:
#   IT STOPS      the transient statuses and only those count as work, and the
#                 component renders nothing when settled. A failure must NOT be
#                 polled - that would hide it behind a spinner.
#   FOUR SCREENS  both tiers' project pages and both tiers' version pages, from
#                 ONE definition. A screen with its own idea of "working" is a
#                 screen that goes stale on its own.
#   IT IS MOUNTED asserted as a rendered element, not an import. A mutation that
#                 deleted the JSX and left the import passed the first version of
#                 that check.
set -uo pipefail
...
ZIP=/tmp/inductionvideo_liveupdate_deploy.zip
...
npx next build >/tmp/inductionvideo_liveupdate_build.log 2>&1 || { tail -30 /tmp/inductionvideo_liveupdate_build.log; fail "build failed"; }
...
for t in "Company induction content" "Leave it out of this project" \
         ...
         "This page updates on its own" "Writing the script"; do
