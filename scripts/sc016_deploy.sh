#!/usr/bin/env bash
# SC-016 production CODE deploy (Mandatory Action Assignment). The additive
# migration 20260729120000_add_audit_templates + template seed must already be
# applied (run scripts/sc016_migrate.sh first). Same proven flow as SC-005..012.
set -uo pipefail
...
echo "== SC-016 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-016 code IS present in this artifact..."
[ -f services/notifications/notificationEventService.ts ] \
  && [ -f components/platform/NotificationPoller.tsx ] \
  && [ -f app/api/platform/notifications/count/route.ts ] \
  && grep -q 'NotificationEvent' prisma/schema.prisma \
  && grep -q 'ACTION_ASSIGNED_TO_ME' services/notifications/notificationTypes.ts \
  && echo "      confirmed: event service + poller + count endpoint + groups present." \
  || { echo "ERROR: SC-016 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-016 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
