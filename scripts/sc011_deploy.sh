#!/usr/bin/env bash
# SC-011 production CODE deploy (Digital Induction Acceptance). The additive
# migration 20260728180000_add_induction_signature must already be applied (run
# scripts/sc011_migrate.sh first). Same proven flow as SC-005..010.
set -uo pipefail
...
echo "== SC-011 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-011 code IS present in this artifact..."
[ -f services/inductionSignature/signatureService.ts ] \
  && [ -f components/checkin/SignaturePad.tsx ] \
  && [ -f components/checkin/AcceptSignStep.tsx ] \
  && [ -f app/worker/inductions/page.tsx ] \
  && [ -f app/api/worker/inductions/\[id\]/signature/route.ts ] \
  && grep -q 'buildSignatureRecord' services/submissions/submissionService.ts \
  && echo "      confirmed: signature service + Accept & Sign + inductions history present." \
  || { echo "ERROR: SC-011 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-011 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
