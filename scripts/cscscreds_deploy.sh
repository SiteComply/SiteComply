#!/usr/bin/env bash
# CSCS CREDENTIALS — the operative card check gets all four stored Smart Check credentials.
#
# resolveCscsProvider passed only URL + key; the stored username and password
# never reached the provider, so every operative check failed "not configured".
#
...
echo "[3/8] Asserting the resolved provider carries all four credentials..."
grep -q 'username: settings.username,' services/cscs/index.ts || fail "buildCscsProvider drops the username"
grep -q 'password: settings.password,' services/cscs/index.ts || fail "buildCscsProvider drops the password"
grep -q "username: config.username ?? ''," services/cscs/index.ts || fail "resolveCscsProvider drops the username"
grep -q "password: config.password ?? ''," services/cscs/index.ts || fail "resolveCscsProvider drops the password"
...
npx next build >/tmp/cscscreds_build.log 2>&1 || {
  tail -30 /tmp/cscscreds_build.log; fail "build failed";
}
...
# Anchored to the two call sites. A bare "username:x.username,password:x.password"
# also matches an unrelated URL helper in another chunk, and would pass without
# the fix.
grep -rqE 'apiKey:[a-zA-Z_$]+\.apiKey,username:[a-zA-Z_$]+\.username,password:[a-zA-Z_$]+\.password\}\);case"mock"' .next/server 2>/dev/null \
  || fail "the built buildCscsProvider does not pass username+password"
grep -rqE 'apiKey:[a-zA-Z_$]+\.apiKey\?\?"",username:[a-zA-Z_$]+\.username\?\?"",password:[a-zA-Z_$]+\.password\?\?""' .next/server 2>/dev/null \
  || fail "the built resolveCscsProvider does not pass username+password"
echo "  ok   the credential pass-through is in the build"
