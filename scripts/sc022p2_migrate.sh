#!/usr/bin/env bash
# SC-020 Phase 1 production migration — applies 20260802140000_add_permission_templates.
#
...
echo "== SC-022 PHASE 2 PRODUCTION MIGRATION =="
...
# The built-in Contractor (standard) template MUST be seeded: the Access tab's
# "Apply Contractor (standard)" button reads the row, and Phase 1 shipped that
# button already. Idempotent — upserts by name.
echo "      seeding the built-in permission template..."
DATABASE_URL="$DBURL" npx tsx scripts/seed-permission-templates.ts 2>&1 | sed 's/^/      /'

echo "[5/5] Verifying the new objects..."
DATABASE_URL="$DBURL" npx tsx scripts/sc022p2_verify.ts

echo "== SC-022 PHASE 2 MIGRATION COMPLETE =="
