...
(async () => {
  ...
  for (const w of WIDTHS) {
    ...
    await p.goto(`${BASE}?sort=site&dir=asc&item=${itemId}`, { waitUntil: 'networkidle', timeout: 120000 });
    await p.waitForTimeout(250);

    console.log(`--- ${w}px (rail open) ---`);

    const railOpen = await p.locator('aside section').count();
    chk('rail is actually open', railOpen > 0, `${railOpen} rail(s)`);
