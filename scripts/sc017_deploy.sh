#!/usr/bin/env bash
# SC-017 production CODE deploy (Mandatory Action Assignment). The additive
# migration 20260729120000_add_audit_templates + template seed must already be
# applied (run scripts/sc017_migrate.sh first). Same proven flow as SC-005..012.
set -uo pipefail
...
echo "== SC-017 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-017 code IS present in this artifact..."
[ -f components/platform/PhotoAnnotator.tsx ] \
  && [ -f lib/annotationRender.ts ] \
  && [ -f lib/imagePrep.ts ] \
  && [ -f services/annotations/annotationTypes.ts ] \
  && grep -q 'annotationData' prisma/schema.prisma \
  && grep -q 'heic2any' package.json \
  && echo "      confirmed: annotator + render + image prep + schema + decoder present." \
  || { echo "ERROR: SC-017 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-017 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
