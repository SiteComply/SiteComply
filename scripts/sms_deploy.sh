...
[ -f services/compliance/occurrenceGenerator.ts ] \
  && [ -f services/sms/smsSendService.ts ] \
  && [ -f components/admin/SmsActivityLog.tsx ] \
  && grep -q 'SmsMessageLog' prisma/schema.prisma \
  && grep -q 'sendingEnabled' prisma/schema.prisma \
  && grep -q 'api.twilio.com' services/sms/twilioProvider.ts \
  && ! grep -q 'Twilio provider is not implemented' services/sms/twilioProvider.ts \
  && grep -q 'sendAuditedSms' services/auth/otpService.ts \
  && grep -q 'messagingServiceSid' services/sms/providerCatalog.ts \
  && echo "      confirmed: real Twilio provider + audited send path + master switch + log UI present." \
  || { echo "ERROR: Twilio SMS code missing — aborting"; exit 1; }
