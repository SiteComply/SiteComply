...
function main() {
  ...
  // Branding is now a publisher's imprint in the colophon and nothing else: the
  // head rule is gone and the drawn substitute mark is replaced by the genuine
  // artwork. These assert the identity's PLACE and RESTRAINT rather than any
  // particular mark, so the next refinement does not break them.
  //
  // Comments are stripped before the body scan — the colophon's own comment
  // explains what lives there and names SiteComply while doing so, which a raw
  // scan reads as a violation.
  const bodyCode = page
    .slice(page.indexOf('cpp-issuer'), page.indexOf('cpp-foot'))
    .replace(/\{\/\*[\s\S]*?\*\/\}/g, '')
    .replace(/\/\*[\s\S]*?\*\//g, '');
  chk('[5] CONTROL — that slice covers the whole document body',
    bodyCode.includes('cpp-doctype') && bodyCode.includes('cpp-approval'));
  chk('[5] SiteComply appears NOWHERE in the document body',
    !bodyCode.includes('SiteComply') && !bodyCode.includes('sitecomply-logo'));
  chk('[5] the GENUINE artwork is used, not a drawn substitute',
    /src="\/sitecomply-logo\.png"/.test(page) &&
    !/<circle cx="16" cy="16"/.test(page));
  chk('[5] the mark is small — 26px, an imprint rather than a logo at scale',
    /\.cpp-foot \.imprint img \{ display: block; height: 26px/.test(doc));
  chk('[5] the attribution carries the name legibly beside it',
    /Prepared and issued in SiteComply/.test(page));
  ...
  chk('[5b] document control keeps its tint and dark-blue rule',
    /--cpp-tint: #f2fafd/.test(doc) && /--cpp-accent: #003a54/.test(doc));
  chk('[5b] no coloured status banner',
