#!/usr/bin/env bash
# SC-038 — production CODE deploy: tab selection carried without colour.
#
# Third pass at the same strip. The first two failed identically: a filled
# brand swatch reads as a primary action wherever it appears, so flooded and
# tinted both landed the active tab next to the Save button in the eye's
# filing system. This one drops colour from the selected state entirely and
# signals it with surface, an inset boundary ring and type weight.
#
...
echo "== SC-038 — QUIETER ADMIN TAB STRIP =="
...
if [ -n "$TOUCHED" ]; then
  ...
  echo "$TOUCHED" | sed 's/^/         /'
  echo "       SC-038 is a colour change. If the devCode suppression has been"
...
OUTSIDE=$(git diff --name-only "${BASE_COMMIT}"..HEAD \
  ...
  | grep -v '^scripts/' || true)
if [ -n "$OUTSIDE" ]; then
  echo "ERROR: SC-038 is a visual refinement to the tab strip, but these also changed:"
  echo "$OUTSIDE" | sed 's/^/         /'
  exit 1
fi
...
// ---------- the subtle tone must carry selection WITHOUT colour ----------
if(nav.indexOf('bg-surface text-ink font-bold shadow-sm ring-1 ring-inset ring-line') < 0)
  fail('the neutral active treatment is missing — surface, boundary and weight are the whole point of this release');
...
if(nav.indexOf('ring-1 ring-inset ring-line/60') < 0)
  fail('inactive segments lost their boundary ring — the strip reads as three words rather than three segments');
// INSET, because a border would change the box model and this release must not
// move anything. Asserted rather than assumed.
if(nav.indexOf('ring-inset') < 0)
  fail('the boundary is no longer an inset ring — a border would resize every segment and this is a visual refinement only');
if(nav.indexOf('border border-transparent') < 0)
  fail('the subtle container is no longer transparent — the filled box is most of the weight the previous pass removed');
...
npm run build > /tmp/sc038_build.log 2>&1
if [ $? -ne 0 ]; then
  echo "ERROR: build failed. Last lines:"
  tail -25 /tmp/sc038_build.log | sed 's/^/         /'
  exit 1
fi
grep -qE "Compiled successfully" /tmp/sc038_build.log || { echo "ERROR: build did not report success"; exit 1; }
...
for needle in 'ring-inset' 'bg-brand-500' 'px-3' 'px-5'; do
...
if [ "$RC" = "200" ] && [ "$ROUTES_OK" = "1" ]; then
  echo "== SC-038 DEPLOYED =="
  echo "   Admin tab selection is surface + boundary + weight, no colour."
  echo "   The four Platform filter strips are unchanged."
else
  echo "== SC-038 DEPLOYED BUT NOT HEALTHY — INVESTIGATE =="
  exit 1
fi
