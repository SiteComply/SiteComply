#!/usr/bin/env bash
# SC-014 production CODE deploy (Customisable Audit Scoring). The additive
# migration 20260729120000_add_audit_templates + template seed must already be
# applied (run scripts/sc014_migrate.sh first). Same proven flow as SC-005..012.
set -uo pipefail
...
echo "== SC-014 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-014 code IS present in this artifact..."
[ -f services/audits/auditScoringService.ts ] \
  && [ -f services/audits/scoringMath.ts ] \
  && [ -f services/audits/scoringConstants.ts ] \
  && [ -f components/platform/AuditScoringConfig.tsx ] \
  && [ -f components/platform/ScoreBreakdownDonut.tsx ] \
  && [ -f components/platform/AuditChecklistPanel.tsx ] \
  && [ -f app/platform/dashboard/audits/\[id\]/scoring/page.tsx ] \
  && [ -f app/api/platform/audits/\[id\]/scoring/route.ts ] \
  && grep -q 'scoringEnabled' prisma/schema.prisma \
  && echo "      confirmed: scoring service + maths + config screen + APIs present." \
  || { echo "ERROR: SC-014 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-014 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
