#!/usr/bin/env bash
# CPP DOWNLOAD CONTROL — same weight as Print, and just "Download".
#
# Download was a solid dark button sitting beside "Edit in Project setup",
# which made the routine act of fetching a document read as the page's primary
# purpose - louder than the edit action next to it. It now takes the SHARED
# secondary treatment via buttonClasses, the same weight Print already has,
# because they are the same kind of action.
#
# The label drops to "Download". The masthead states the revision two lines
# below and the file announces itself as a PDF; a button repeating both was
# noise.
#
# BEHAVIOUR IS UNCHANGED. Download still appears only for an ISSUED or
# SUPERSEDED revision and Print only for a working draft - they remain mutually
# exclusive in one slot. Showing browser Print on an issued revision would put
# back the browser headers and footers this whole workstream removed.
#
# NO MIGRATION.
#
set -uo pipefail
...
ZIP=/tmp/cppdownload_deploy.zip
...
# THE DOCUMENT ACTIONS. Same weight, same slot, no noise in the label.
python3 - "$PAGE" components/worker/PrintButton.tsx <<'PYACT' || exit 1
import io, re, sys
def die(m):
    print("  FAIL " + m); sys.exit(1)
def strip(t):
...
page = strip(io.open(sys.argv[1], encoding='utf-8').read())
pb = io.open(sys.argv[2], encoding='utf-8').read()

m = re.search(r'<a\s+href=\{`/api/platform/sites[\s\S]*?</a>', page)
if not m: die("the download control is gone")
dl = m.group(0)
if "buttonClasses({ variant: 'secondary'" not in dl:
    die("the download control no longer takes the shared secondary treatment")
if 'bg-ink' in dl or 'font-semibold' in dl:
    die("the download control is a solid primary-weight button again")
# Taken from between the opening tag and </a>. An earlier version split on the
# first '>' and then required another one, so it always captured an empty string
# and the check below could never fire.
lm = re.search(r'>\s*([^<>]+?)\s*</a>', dl, re.S)
if not lm:
    die("the download control has no readable label")
label = lm.group(1).strip()
if not label:
    die("the download control's label is empty")
if re.search(r'Revision|PDF|\$\{', label):
    die('the download label repeats what the page already says: "%s"' % label)
if "variant = 'secondary'" not in pb:
    die("PrintButton's default weight changed - the two actions would no longer match")
pbtag = re.search(r'<PrintButton[^>]*/>', page)
if pbtag and 'variant=' in pbtag.group(0):
    die("Print overrides its variant - the two document actions would no longer match")
if page.index('Edit in Project setup') > page.index('/pdf'):
    die("the download control moved out of the right-hand action group")
print('  ok   document actions: matched secondary weight, label "%s"' % label)
PYACT

# PAGINATION. Asserted at source here; the suite proves it by RENDERING and
# reading back which page things landed on.
python3 - services/sites/cppPdf/CppPdfDocument.tsx <<'PYPAG' || exit 1
import io, re, sys
...
# NO forced break anywhere in the document. Matching the contents wrapper by its
# style string was the first attempt and could never fire - the style had gained
# a marginBottom, so the pattern no longer described the code it guarded.
brk = re.findall(r'\sbreak(?=[\s>=])', src)
if brk:
    die("a forced page break is back (%d found) - that is what left sheets half empty" % len(brk))
if not re.search(r'<View wrap=\{false\}>\s*<Text style=\{s\.part\}>', src):
    die("the part heading is no longer bound to its first section - it can be stranded at a page foot")
if 'minPresenceAhead' in src:
    die("minPresenceAhead is back - it was measured here and did not hold")
if 'marginTop: 26, marginBottom: 30' not in src:
    die("the contents lost its bottom margin - the part heading will sit hard against the last row")
# Nothing that must stay whole may become splittable.
for pat, what in ((r'style=\{s\.section\} key=\{sec\.key\} wrap=\{false\}', 'a numbered section'),
                  (r'style=\{s\.section\} wrap=\{false\}', 'the drawings block'),
                  (r'style=\{s\.approval\} wrap=\{false\}', 'the approval block')):
    if not re.search(pat, src):
        die("%s can now split across a sheet" % what)
print("  ok   pagination: contents on page 1, plan follows, heading never stranded")
PYPAG
...
# Comments stripped: the note explaining the removal names the class it removed.
grep -v '^[[:space:]]*\*' "$PAGE" | grep -v '^[[:space:]]*//' | grep -q 'className="w-44"' \
  && fail "the fixed-width print block is back"
python3 - "$PAGE" <<'PYBTN' || exit 1
import io, re, sys
src = io.open(sys.argv[1], encoding='utf-8').read()
def die(m):
    print("  FAIL " + m); sys.exit(1)
m = re.search(r'<PrintButton[^>]*/>', src)
if not m: die("the print control is gone")
btn = m.group(0)
if 'variant=' in btn:
    die("the print control overrides the variant - it should take the standard secondary treatment")
lm = re.search(r'label="([^"]*)"', btn)
if not lm: die("the print control has no label")
label = lm.group(1)
if re.search(r'browser|window\.print|print-to|PDF', label, re.I):
    die('the label names the mechanism again: "%s"' % label)
if len(label) > 24:
    die('the label is a sentence, not a control: "%s"' % label)
print('  ok   print control: standard secondary, natural width, label "%s"' % label)
PYBTN
...
npx next build >/tmp/cppdownload_build.log 2>&1 || {
  tail -30 /tmp/cppdownload_build.log; fail "build failed";
}
