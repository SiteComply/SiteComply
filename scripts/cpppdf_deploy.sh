#!/usr/bin/env bash
# CPP SERVER-RENDERED PDF FOR ISSUED REVISIONS — production deploy.
#
# The plan was produced with window.print(), which stamps the browser's own page
# title, URL, timestamp and "1/7" onto a document issued to clients and read by
# CDM auditors. Those come from a checkbox in each reader's print dialog; no CSS
# reaches them, and @page margin boxes are not implemented in Chrome. So the
# document is rendered on the server, where the running head, the running foot
# and the page numbers are ours and are true.
#
#   - ISSUED AND SUPERSEDED REVISIONS ONLY, rendered from the frozen snapshot.
#     A draft has no PDF: one would look exactly like an approved plan.
#   - DETERMINISTIC. The PDF's creation date is pinned to the revision's
#     issuedAt, so the same revision renders byte-identical every time. That is
#     what makes rendering on demand equivalent to storing the file.
#   - NO SCHEMA CHANGE, NO BLOB, NO MIGRATION. CppRevision.snapshot already
#     holds the frozen document.
#
# TWO SILENT TRAPS ARE GUARDED HERE because both produced a plausible document:
#   - a unitless lineHeight on the Page makes every `render`-prop element
#     resolve to NOTHING — no page numbers, no running head, no error;
#   - WOFF2 fonts read fine and then cannot be embedded, dying inside pdfkit.
#
set -uo pipefail
...
# The browser-print claim must be gone from the plan screen.
grep -q "Print / save as PDF" "$PAGE" && fail "the plan still offers browser print as its PDF"
grep -q "Print draft (browser)" "$PAGE" || fail "the draft print control is gone or unlabelled"
...
npx next build >/tmp/cpppdf_build.log 2>&1 || {
  tail -30 /tmp/cpppdf_build.log; fail "build failed";
}
