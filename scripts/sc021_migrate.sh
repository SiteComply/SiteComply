#!/usr/bin/env bash
# SC-020 Phase 1 production migration — applies 20260801090000_add_site_service_availability.
#
...
echo "== SC-021 PHASE 1 PRODUCTION MIGRATION =="
...
# SC-021 adds Temporary Works Inspection and Environmental Inspection to the
# starter library, so the seed MUST run or the two types the requirement names
# would not exist in production. The seed is idempotent (upsert by name +
# isSystem) and does not touch historic audits, which snapshot their items at
# creation.
echo "      seeding audit templates (adds the two SC-021 inspection types)..."
DATABASE_URL="$DBURL" npx tsx scripts/seed-audit-templates.ts 2>&1 | sed 's/^/      /'


echo "[5/5] Verifying the new objects..."
DATABASE_URL="$DBURL" npx tsx scripts/sc021_verify.ts

echo "== SC-021 PHASE 1 MIGRATION COMPLETE =="
