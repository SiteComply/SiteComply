#!/usr/bin/env bash
# CSCS MULTI-CARD — a holder with several cards (renewal, two types) can verify:
# VALID when at least one card is current. Previously any second card = ERROR.
#
...
ZIP=/tmp/cscsmulticard_deploy.zip
...
echo "[3/8] Asserting the multi-card rule and the earlier fixes..."
M=services/cscs/smartCheckMapper.ts
grep -q "const current = cards.filter((_, i) => standings\[i\] === 'VALID');" "$M" || fail "the current-card rule is missing"
grep -q "if (holders.size > 1) {" "$M" || fail "different holders are no longer refused"
grep -q "More than one card matched these details" "$M" && fail "the blanket multi-card ERROR is back"
grep -q "permits(viewer.role, 'checkins', 'export')" "app/api/platform/workers/[id]/cscs-check/route.ts" || fail "check-now permission regressed"
...
npx next build >/tmp/cscsmulticard_build.log 2>&1 || {
  tail -30 /tmp/cscsmulticard_build.log; fail "build failed";
}
...
grep -rqF "cards returned: " .next/server 2>/dev/null || fail "the multi-card audit note is not in the build"
grep -rqF "name different people" .next/server 2>/dev/null || fail "the different-holders refusal is not in the build"
grep -rqF "More than one card matched these details" .next/server 2>/dev/null && fail "the old blanket ERROR is in the build"
RB=".next/server/app/api/platform/workers/[id]/cscs-check/route.js"
grep -qF '.role,"checkins","export"))return' "$RB" || fail "check-now permission missing from the build"
...
echo "  ok   the multi-card rule and both earlier fixes are in the build"
