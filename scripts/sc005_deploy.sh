#!/usr/bin/env bash
# SC-005 production CODE deploy (commit a873ce6). No database migration — the
# data model already supports multiple concurrent open check-ins per worker, so
# SC-005 is code-only. Same proven flow as the SC-003 / OTP-fix deploys.
set -uo pipefail
...
echo "== SC-005 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-005 code IS present in this artifact..."
[ -f components/checkin/KnowledgeCheck.tsx ] \
  && [ -f services/knowledgeChecks/questionBankService.ts ] \
  && [ -f app/api/worker/knowledge-check/start/route.ts ] \
  && grep -q 'evaluateGate' services/submissions/submissionService.ts \
  && echo "      confirmed: knowledge-check generation + gate + worker UI present." \
  || { echo "ERROR: SC-005 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-005 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
