#!/usr/bin/env bash
# SC-018 production CODE deploy (Mandatory Action Assignment). The additive
# migration 20260729120000_add_audit_templates + template seed must already be
# applied (run scripts/sc018_migrate.sh first). Same proven flow as SC-005..012.
set -uo pipefail
...
echo "== SC-018 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-018 code IS present in this artifact..."
grep -q 'isToolboxTalkItem' services/checklists/inductionFlow.ts \
  && grep -q 'isRetiredInductionItem' services/sites/siteService.ts \
  && ! grep -q 'attended the toolbox talk' services/checklists/ukInductionTemplate.ts \
  && echo "      confirmed: toolbox matcher + render-time filter present, template item removed." \
  || { echo "ERROR: SC-018 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-018 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
