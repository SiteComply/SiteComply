#!/usr/bin/env bash
# SC-039 — production CODE deploy: back to an underlined tab row.
#
# Fourth and last pass at the same strip. A flooded pill, a tinted pill and a
# neutral surface chip were each rejected for the same reason: a filled shape
# reads as a button. This returns to the original text-tab row and fixes the
# discoverability it actually lacked — a 3px indicator, two weight steps
# between active and inactive, and a hover state where the indicator answers.
#
# THE BLAST RADIUS IS NOW A DELETION. AdminTabs no longer renders through
# SegmentedNav, so navUi is reverted to its pre-SC-035 state and the size/tone
# props are gone. [3/7] therefore asserts the Platform strips from the OTHER
# direction than the last three scripts did: not that the defaults still say
# solid, but that the solid treatment is the ONLY treatment left and that the
# now-removed props really are removed rather than half-deleted.
#
...
echo "== SC-039 — QUIETER ADMIN TAB STRIP =="
...
if [ -n "$TOUCHED" ]; then
  ...
  echo "$TOUCHED" | sed 's/^/         /'
  echo "       SC-039 is a colour change. If the devCode suppression has been"
...
OUTSIDE=$(git diff --name-only "${BASE_COMMIT}"..HEAD \
  ...
  | grep -v '^scripts/' || true)
if [ -n "$OUTSIDE" ]; then
  echo "ERROR: SC-039 is a visual refinement to the tab strip, but these also changed:"
  echo "$OUTSIDE" | sed 's/^/         /'
  exit 1
fi
...
// navUi is reverted, so the solid treatment is no longer one branch of two —
// it is the only one. Asserted literally, because a partial revert that left
// a dangling ternary would still compile and would still repaint four strips.
if(nav.indexOf('bg-brand-500 text-white shadow-sm') < 0)
  fail('the solid active fill is gone — this is what the four Platform filter strips render');
...
if(nav.indexOf('border border-line bg-surface-sunken') < 0)
  fail('the recessed container is gone — the Platform filter strips would lose their background');
...
// The IMPORT, not the identifier: the file's own comment explains why it no
// longer delegates, so a bare name check matches the prose and passes.
if(/^import .*SegmentedNav/m.test(tabs))
  fail('AdminTabs imports SegmentedNav again — that component IS a segmented control, and this release is deliberately not one');

if(tabs.indexOf('border-b border-line') < 0)
  fail('the shared tab row rule is gone — the indicator needs a rule to break');
if(tabs.indexOf('border-b-[3px]') < 0)
  fail('the 3px active indicator is gone — it is the whole discoverability fix');
if(tabs.indexOf('border-brand-500') < 0)
  fail('the active indicator is no longer the brand colour');
if(tabs.indexOf('hover:border-ink-subtle') < 0)
  fail('the hover indicator is gone — a tab that does not answer the pointer is what made the original look inert');
if(tabs.indexOf('font-bold') < 0 || tabs.indexOf('font-medium') < 0)
  fail('the two type weights are gone — one step between active and inactive is what made the original read as a list of links');
if(tabs.indexOf('-mb-px') < 0)
  fail('the indicator no longer sits on the row rule — it would read as two stacked lines');

// ---------- the treatments this release exists to REMOVE ----------
for(const banned of ['bg-brand', 'bg-surface', 'rounded-', 'shadow-', 'ring-']){
  if(tabs.indexOf(banned) >= 0)
    fail('AdminTabs uses ' + banned + ' — no pill, no segmented control, no per-tab border and no filled background');
}
" || exit 1
...
npm run build > /tmp/sc039_build.log 2>&1
if [ $? -ne 0 ]; then
  echo "ERROR: build failed. Last lines:"
  tail -25 /tmp/sc039_build.log | sed 's/^/         /'
  exit 1
fi
grep -qE "Compiled successfully" /tmp/sc039_build.log || { echo "ERROR: build did not report success"; exit 1; }
...
for needle in 'border-bottom-width:3px' 'bg-brand-500' 'px-3'; do
  grep -q "$needle" "$CSS" || { echo "ERROR: '$needle' is missing from the built stylesheet"; exit 1; }
done
echo "      stylesheet carries the indicator and the Platform strip utilities."
...
if [ "$RC" = "200" ] && [ "$ROUTES_OK" = "1" ]; then
  echo "== SC-039 DEPLOYED =="
  echo "   Admin tabs are an underlined row again, with a hover indicator."
  echo "   The four Platform filter strips are unchanged."
else
  echo "== SC-039 DEPLOYED BUT NOT HEALTHY — INVESTIGATE =="
  exit 1
fi
