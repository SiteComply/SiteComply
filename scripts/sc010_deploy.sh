#!/usr/bin/env bash
# SC-010 production CODE deploy (Attendance History & Timesheets). CODE-ONLY —
# there is NO migration: attendance history and timesheets are derived from the
# existing Submission records (no new tables). Same proven flow as SC-005..009.
set -uo pipefail
...
echo "== SC-010 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-010 code IS present in this artifact..."
[ -f services/attendance/attendanceHistoryService.ts ] \
  && [ -f app/worker/attendance/page.tsx ] \
  && [ -f app/worker/attendance/history/page.tsx ] \
  && [ -f components/attendance/AttendanceShell.tsx ] \
  && grep -q 'attendanceTotalMinutes' services/reports/attendanceReport.ts \
  && echo "      confirmed: attendance service + worker pages + report column present." \
  || { echo "ERROR: SC-010 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-010 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
