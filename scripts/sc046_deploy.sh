#!/usr/bin/env bash
# SC-046 — production CODE deploy: Access Requests pending badge.
#
# The badge, the count and the refresh were already built; the layout gated
# them on isNotificationEnabled('platform_access_request'), a key removed from
# the notification catalogue. That helper returns false for unknown types, so
# the gate was permanently closed and the count forced to 0.
#
# ONE EXPRESSION plus a now-unused import.
#
# THE RISK IS THE BADGE SHOWING THE WRONG NUMBER. [3/7] asserts the count is
# PENDING-only, that the badge still hides at zero, and that the layout stays
# force-dynamic — without that it would cache and stop tracking approvals.
#
...
echo "== SC-046 — ACCESS REQUESTS PENDING BADGE =="
...
if [ -n "$TOUCHED" ]; then
  ...
  echo "$TOUCHED" | sed 's/^/         /'
  echo "       SC-046 is a colour change. If the devCode suppression has been"
...
OUTSIDE=$(git diff --name-only "${BASE_COMMIT}"..HEAD \
  | grep -v '^app/admin/(dashboard)/layout.tsx$' \
  | grep -v '^scripts/' || true)
if [ -n "$OUTSIDE" ]; then
  echo "ERROR: SC-046 fixes ONE defect, but these also changed:"
  echo "$OUTSIDE" | sed 's/^/         /'
  exit 1
fi
...
const layout=strip('app/admin/(dashboard)/layout.tsx');
const nav=strip('components/admin/AdminNav.tsx');
const svc=strip('services/accessRequests/accessRequestService.ts');

// ---------- the gate must be gone ----------
if(layout.indexOf('isNotificationEnabled') >= 0)
  fail('the badge is gated on a notification type again — that gate is what suppressed it entirely');
if(layout.indexOf('const pendingAccessRequests = await countPendingAccessRequests();') < 0)
  fail('the layout no longer counts pending access requests unconditionally');

// ---------- the layout must stay dynamic, or the badge goes stale ----------
if(layout.indexOf('export const dynamic = force-dynamic;') < 0)
  fail('the layout is no longer force-dynamic — the badge would cache and stop updating as requests are actioned');

// ---------- PENDING only ----------
if(svc.indexOf('where: { status: AccessRequestStatus.PENDING },') < 0)
  fail('the count is no longer restricted to PENDING — approved or rejected requests would inflate the badge');

// ---------- the badge must still hide at zero, and stay a badge ----------
if(nav.indexOf('pendingAccessRequests > 0') < 0)
  fail('the badge no longer hides when there is nothing pending');
if(nav.indexOf('bg-danger-500') < 0)
  fail('the badge is no longer the red attention colour');
if(nav.indexOf('aria-label={') < 0)
  fail('the badge lost its accessible label — a bare number beside a link says nothing to a screen reader');
" || exit 1

# The Access Requests page and the approval flow must not have been touched.
PAGE_TOUCHED=$(git diff --name-only "${BASE_COMMIT}"..HEAD -- 'app/admin/(dashboard)/platform-access-requests' services/accessRequests app/api/admin/platform-access-requests)
if [ -n "$PAGE_TOUCHED" ]; then
  echo "ERROR: the Access Requests page or approval flow changed:"
  echo "$PAGE_TOUCHED" | sed 's/^/         /'
  echo "       SC-046 is a navigation-awareness change only."
  exit 1
fi
echo "      confirmed: gate removed, PENDING-only, hides at zero, page untouched."
...
npx tsx scripts/sc001_cscs_tests.ts > /tmp/sc046_cscs.log 2>&1
if [ $? -ne 0 ]; then
  echo "ERROR: the CSCS verification suite failed."
  tail -15 /tmp/sc046_cscs.log | sed 's/^/         /'
  exit 1
fi
echo "      $(tail -2 /tmp/sc046_cscs.log | tr -d '\n' | sed 's/.*== //')"

echo "[4/7] Building and packaging..."
npx prisma generate > /dev/null 2>&1 || { echo "ERROR: prisma generate failed"; exit 1; }
npm run build > /tmp/sc046_build.log 2>&1
if [ $? -ne 0 ]; then
  echo "ERROR: build failed. Last lines:"
  tail -25 /tmp/sc046_build.log | sed 's/^/         /'
  exit 1
fi
grep -qE "Compiled successfully" /tmp/sc046_build.log || { echo "ERROR: build did not report success"; exit 1; }
...
if [ "$RC" = "200" ] && [ "$ROUTES_OK" = "1" ]; then
  echo "== SC-046 DEPLOYED =="
  echo "   The Access Requests nav item now shows a red pending count."
  echo "   Hidden at zero; PENDING only; the requests page is unchanged."
else
  echo "== SC-046 DEPLOYED BUT NOT HEALTHY — INVESTIGATE =="
  exit 1
fi
