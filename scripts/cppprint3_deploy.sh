#!/usr/bin/env bash
# CPP DRAFT PRINT CONTROL — standard secondary button, labelled "Print".
#
# The control was a w-44 full-width secondary button reading "Print draft
# (browser)": the footprint of a primary action for what is a convenience, and a
# label that exposed the mechanism. A brief ghost-variant step in between was
# quieter but non-standard.
#
# It is now the ordinary secondary Button at its natural width, reading "Print".
# Whether the bytes come from the browser or from a server-rendered document is
# not a user's concern.
#
# BEHAVIOUR IS UNCHANGED - still window.print(), still the working draft only.
#
# The close-out pack and the CLIENT-FACING share page also use PrintButton and
# are NOT in scope: the component's defaults are the original values and only
# the CPP passes anything, which the assertions below enforce.
#
# NO MIGRATION.
#
set -uo pipefail
...
npx next build >/tmp/cppprint3_build.log 2>&1 || {
  tail -30 /tmp/cppprint3_build.log; fail "build failed";
}
