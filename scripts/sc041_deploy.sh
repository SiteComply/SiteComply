#!/usr/bin/env bash
# SC-041 — production CODE deploy: permit status filter fix (M-3).
#
# The register filtered on the stored status column while the badge showed the
# derived one. EXPIRED is almost never stored — effectiveStatus() derives it
# for an APPROVED permit past its validUntil — so "Expired" returned nothing
# and "Approved" returned permits that all displayed as Expired.
#
# THE RISK IS A FILTER THAT SILENTLY DROPS A CLAUSE. Status and search are
# each an OR, and the old code assigned both to where.OR, so combining them
# would have lost one. [3/7] asserts they are collected into AND instead.
#
# It also pins the blast radius: the pending-approval badge counts
# SUBMITTED/UNDER_REVIEW through its OWN where clause and must not have moved,
# and effectiveStatus itself — which the worker portal and the permit detail
# page also render from — must be untouched. Asserted as a diff.
#
...
echo "== SC-041 — PERMIT STATUS FILTER FIX (M-3) =="
...
if [ -n "$TOUCHED" ]; then
  ...
  echo "$TOUCHED" | sed 's/^/         /'
  echo "       SC-041 is a colour change. If the devCode suppression has been"
...
OUTSIDE=$(git diff --name-only "${BASE_COMMIT}"..HEAD \
  | grep -v '^services/permits/permitAdminService.ts$' \
  | grep -v '^scripts/' || true)
if [ -n "$OUTSIDE" ]; then
  echo "ERROR: SC-041 fixes ONE defect, but these also changed:"
  echo "$OUTSIDE" | sed 's/^/         /'
  exit 1
fi
...
const adm=strip('services/permits/permitAdminService.ts');
const svc=strip('services/permits/permitService.ts');

// ---------- the filter must express the DERIVED status ----------
if(adm.indexOf('status: EXPIRED') < 0 || adm.indexOf('status: APPROVED, validUntil: { lt: now }') < 0)
  fail('the EXPIRED branch is gone — filtering Expired would again return nothing while expired permits sit in the register');
if(adm.indexOf('OR: [{ validUntil: null }, { validUntil: { gte: now } }]') < 0)
  fail('the APPROVED branch no longer excludes lapsed permits — Approved would again return rows that display as Expired');
if(adm.indexOf('where.status = filters.status') >= 0)
  fail('the filter assigns the raw status column again — that is the defect itself');

// ---------- status + search must not clobber each other ----------
if(adm.indexOf('where.AND = and') < 0)
  fail('the clauses are no longer collected into AND — status and search are each an OR and one would overwrite the other');
if(/where\.OR\s*=/.test(adm))
  fail('something still assigns where.OR directly, which is how a clause gets silently dropped');

// ---------- the derivation itself must be UNCHANGED ----------
// The worker portal and the permit detail page render from this too.
if(svc.indexOf('if (\n    p.status === APPROVED &&\n    p.validUntil &&\n    p.validUntil.getTime() < Date.now()\n  ) {') < 0
   && svc.indexOf('p.status === APPROVED &&') < 0)
  fail('effectiveStatus changed — this release must alter only the filter, not what a status DISPLAYS as');

// ---------- the pending badge must not have been dragged in ----------
if(adm.indexOf('status: { in: [SUBMITTED, UNDER_REVIEW] }') < 0)
  fail('the pending-approval badge query changed — it never involved a derived status and is out of scope');
" || exit 1

# effectiveStatus and the worker-side permit list live in permitService.ts.
# This release must not have touched that file at all.
SVC_TOUCHED=$(git diff --name-only "${BASE_COMMIT}"..HEAD -- services/permits/permitService.ts)
if [ -n "$SVC_TOUCHED" ]; then
  echo "ERROR: permitService.ts changed. effectiveStatus() is rendered by the worker"
  echo "       portal and the permit detail page; this release changes only the filter."
  exit 1
fi
echo "      confirmed: filter matches the derived status, clauses combine, display logic untouched."
...
npm run build > /tmp/sc041_build.log 2>&1
if [ $? -ne 0 ]; then
  echo "ERROR: build failed. Last lines:"
  tail -25 /tmp/sc041_build.log | sed 's/^/         /'
  exit 1
fi
grep -qE "Compiled successfully" /tmp/sc041_build.log || { echo "ERROR: build did not report success"; exit 1; }
...
if [ "$RC" = "200" ] && [ "$ROUTES_OK" = "1" ]; then
  echo "== SC-041 DEPLOYED =="
  echo "   Permit filtering now matches the status shown to users."
  echo "   Nothing else changed."
else
  echo "== SC-041 DEPLOYED BUT NOT HEALTHY — INVESTIGATE =="
  exit 1
fi
