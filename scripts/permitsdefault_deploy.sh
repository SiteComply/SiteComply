#!/usr/bin/env bash
# PERMITS ON BY DEFAULT — a site that was never asked now shows the Permits
# panel to operatives; an explicit "off" on record still wins.
#
# NO MIGRATION: sites with no stored setting inherit the default.
set -uo pipefail
...
grep -q "defaultEnabled: true," services/workerDashboard/dashboardPanels.ts || fail "the permits default regressed"
