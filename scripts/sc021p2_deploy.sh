...
[ -f services/compliance/occurrenceGenerator.ts ] \
  ...
  && [ -f app/platform/dashboard/sites/config-templates/page.tsx ] \
  ...
  && grep -q 'mandatoryPermitTypeIds' services/siteServices/siteServiceAvailability.ts \
  && grep -q 'configTemplateId' services/sites/platformSiteService.ts \
  && echo "      confirmed: Phase 1 enforcement + templates + policy + library + creation-time apply present." \
