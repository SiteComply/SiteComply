#!/usr/bin/env bash
# SC-008 production CODE deploy (Site Information). The additive migration
# 20260728160000_add_site_information must already be applied (run
# scripts/sc008_migrate.sh first). Same proven flow as SC-005/006/007.
set -uo pipefail
...
echo "== SC-008 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-008 code IS present in this artifact..."
[ -f services/sites/siteInformationService.ts ] \
  && [ -f app/api/platform/sites/\[id\]/site-information/route.ts ] \
  && [ -f app/api/platform/sites/\[id\]/site-map/route.ts ] \
  && [ -f app/api/worker/site-map/route.ts ] \
  && [ -f components/platform/SiteInformationConfig.tsx ] \
  && grep -q 'getWorkerSiteInformation' app/worker/site-information/page.tsx \
  && echo "      confirmed: site-information service + APIs + admin panel + worker page present." \
  || { echo "ERROR: SC-008 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-008 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
