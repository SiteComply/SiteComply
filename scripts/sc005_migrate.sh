#!/usr/bin/env bash
# SC-005 production migration — applies 20260724150000_add_knowledge_checks.
#
...
echo "== SC-005 PRODUCTION MIGRATION =="
...
(async () => {
  const rows = await p.$queryRawUnsafe(`
    ...
    WHERE table_schema = '"'"'public'"'"'
      AND table_name IN ('"'"'SiteInductionConfig'"'"','"'"'InductionQuestionBank'"'"','"'"'InductionQuestion'"'"','"'"'KnowledgeCheckAttempt'"'"','"'"'QuestionFlag'"'"')
    ORDER BY table_name`);
  const cols = await p.$queryRawUnsafe(`
    SELECT column_name FROM information_schema.columns
    WHERE table_schema = '"'"'public'"'"' AND table_name = '"'"'JobSite'"'"'
      AND column_name IN ('"'"'knowledgeCheckPassed'"'"','"'"'knowledgeCheckSkipped'"'"')
    ORDER BY column_name`);
  console.log("      tables:", rows.map(r => r.table_name).join(", ") || "(none)");
  console.log("      JobSite columns:", cols.map(c => c.column_name).join(", ") || "(none)");
  const ok = rows.length === 5 && cols.length === 2;
...
echo "== SC-005 MIGRATION COMPLETE =="
