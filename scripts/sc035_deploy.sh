#!/usr/bin/env bash
# SC-035 — production CODE deploy: Integrations tab strip polish.
#
...
APP=sitecomply-web
PG=sitecomply-pg
PRECHECK_RULE=tmp-sc035-precheck
SCM="https://${APP}.scm.azurewebsites.net"
BASE="https://${APP}.azurewebsites.net"
HEALTH="${BASE}/api/health"
ZIP=/tmp/sc035_deploy.zip
...
echo "== SC-035 — INTEGRATIONS TAB POLISH =="
...
if [ "$PRESENT" != "2" ]; then
  ...
  echo "       RUN scripts/sc035_migrate.sh FIRST."
  exit 1
fi
...
OUTSIDE=$(git diff --name-only "${BASE_COMMIT}"..HEAD \
  ...
  | grep -v '^scripts/' || true)
if [ -n "$OUTSIDE" ]; then
  echo "ERROR: SC-035 is a visual change to the tab strip, but these also changed:"
  echo "$OUTSIDE" | sed 's/^/         /'
  exit 1
fi
...
// ---------- SC-035 tab strip polish ----------
// NOTE: no double quotes and no backticks in comments inside this node -e block.
const tsTabs=strip('components/admin/AdminTabs.tsx');
const tsNav=strip('components/platform/navUi.tsx');
const tsPage=strip('app/admin/(dashboard)/settings/integrations/page.tsx');

// ---- the strip must DELEGATE, not carry its own copy of the styling ----
// Two hand-maintained copies of one visual language drift the moment either is
// touched. If AdminTabs starts styling itself again, that has begun.
if(tsTabs.indexOf('SegmentedNav') < 0)
  fail('AdminTabs no longer renders through SegmentedNav — the two portals would drift into different tab strips');
if(tsTabs.indexOf('border-b-2') >= 0)
  fail('AdminTabs is hand-rolling the old underlined strip again');

// ---- THE BLAST RADIUS. SegmentedNav is now shared by both portals ----
// The size must DEFAULT to the small rendering, or the four Platform filter
// strips silently grow. This is the only way this change reaches beyond the
// Admin Centre.
if(tsNav.indexOf('size = ' + String.fromCharCode(39) + 'sm' + String.fromCharCode(39)) < 0)
  fail('SegmentedNav size no longer defaults to sm — the Platform filter strips would change size');
if(tsNav.indexOf('px-3 py-1.5') < 0)
  fail('the small SegmentedNav padding changed — the Platform filter strips would move');
if(tsTabs.indexOf('size=') < 0)
  fail('AdminTabs no longer opts into the larger size');

// ---- the active state must be a FILL, not just an underline ----
if(tsNav.indexOf('bg-brand-500 text-white') < 0)
  fail('the active segment is no longer a filled pill — the tabs would read as links again');
// The container is what frames the set as a control.
if(tsNav.indexOf('bg-surface-sunken') < 0)
  fail('the segmented container background is gone');

// ---- everything the previous release guaranteed still holds ----
for(const c of ['SmsProviderSettings','SmsActivityLog','CscsProviderSettings','AiProviderSettings']){
  if(tsPage.indexOf(c) < 0) fail(c + ' is no longer rendered');
}
if(tsPage.indexOf('TABS.some') < 0) fail('an unknown tab is no longer validated');
if(tsPage.indexOf('ReadOnlyBanner') < 0) fail('the read-only banner for VIEWER admins is gone');
if(tsTabs.indexOf('use client') >= 0)
  fail('AdminTabs became a client component');

console.log('      confirmed: the strip delegates to SegmentedNav, the shared');
console.log('                 size still defaults to sm so Platform strips cannot');
console.log('                 move, the active segment is a filled pill, and every');
console.log('                 settings panel and gate is still in place.');
...
for path in \
  ...
  # pattern. SC-035 printed "routes: all reachable" while every route was
...
if [ "$CODE" = "200" ] && [ -z "$SMOKE_FAIL" ]; then
  echo "== SC-035 DEPLOYED =="
  echo "   Visual only. Routes, query-string behaviour and every setting unchanged."
else
