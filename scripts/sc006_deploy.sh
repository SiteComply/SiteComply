#!/usr/bin/env bash
# SC-006 production CODE deploy (commit a873ce6). No database migration — the
# data model already supports multiple concurrent open check-ins per worker, so
# SC-006 is code-only. Same proven flow as the SC-003 / OTP-fix deploys.
set -uo pipefail
...
echo "== SC-006 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-006 code IS present in this artifact..."
[ -f services/induction/inductionValidityService.ts ] \
  && [ -f app/api/worker/express-checkin/route.ts ] \
  && [ -f components/platform/InductionValidityConfig.tsx ] \
  && grep -q 'getInductionValidity' app/check-in/site/\[siteId\]/page.tsx \
  && echo "      confirmed: induction validity + express check-in + controls present." \
  || { echo "ERROR: SC-006 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-006 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
