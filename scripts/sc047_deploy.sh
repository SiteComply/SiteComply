#!/usr/bin/env bash
# SC-047 — production CODE deploy: one owner for notification settings (F-1).
#
# Two surfaces wrote the same singleton row through two write paths, and they
# disagreed: the Admin Centre writer rebuilt `settings` from the catalogue and
# so dropped __thresholds, silently resetting reminder lead times a Director
# had set. The Platform surface is now the sole owner; the Admin Centre keeps
# a read-only fallback, exactly as company profile did.
#
# THE RISK IS LOSING A SETTING OR A PERMISSION. [3/7] asserts there is exactly
# ONE upsert to the row, that the surviving writer still MERGES (so thresholds
# survive), that the Admin write path is retired rather than silently accepting
# writes, and that the Platform edit permission is unchanged.
#
...
echo "== SC-047 — NOTIFICATION SETTINGS: SINGLE OWNER =="
...
if [ -n "$TOUCHED" ]; then
  ...
  echo "$TOUCHED" | sed 's/^/         /'
  echo "       SC-047 is a colour change. If the devCode suppression has been"
...
OUTSIDE=$(git diff --name-only "${BASE_COMMIT}"..HEAD \
  | grep -v '^app/admin/(dashboard)/settings/notifications/page.tsx$' \
  | grep -v '^app/admin/(dashboard)/settings/page.tsx$' \
  | grep -v '^app/api/admin/settings/notifications/route.ts$' \
  | grep -v '^components/admin/NotificationSettings.tsx$' \
  | grep -v '^services/notifications/notificationConfigService.ts$' \
  | grep -v '^scripts/' || true)
if [ -n "$OUTSIDE" ]; then
  echo "ERROR: SC-047 fixes ONE defect, but these also changed:"
  echo "$OUTSIDE" | sed 's/^/         /'
  exit 1
fi
...
const svc=strip('services/notifications/notificationConfigService.ts');
const route=strip('app/api/admin/settings/notifications/route.ts');
const page=strip('app/admin/(dashboard)/settings/notifications/page.tsx');

// ---------- EXACTLY ONE WRITER ----------
const writes=(svc.match(/notificationConfig\.(upsert|update|create|delete)/g)||[]).length;
if(writes !== 1)
  fail('there are '+writes+' write paths to the notifications row — the whole point of this release is that there is exactly one');
if(svc.indexOf('export async function savePlatformNotificationSettings(') < 0)
  fail('the surviving owner writer is gone');
if(svc.indexOf('export async function saveNotificationConfig(') >= 0)
  fail('the second (Admin Centre) writer is back — that is the duplication itself');

// ---------- the surviving writer must still MERGE ----------
// Rebuilding from the catalogue is what dropped the Director thresholds.
if(svc.indexOf('const next: Record<string, unknown> = { ...existing };') < 0)
  fail('the owner writer no longer merges over the stored object — reminder thresholds would be dropped again');
if(svc.indexOf('next[THRESHOLD_KEY] = {') < 0)
  fail('the owner writer no longer persists the reminder thresholds');

// ---------- the Admin write path must be RETIRED, not silently accepting ----------
if(route.indexOf('status: 409') < 0)
  fail('the retired admin route no longer answers 409 — an old tab would think its save worked');
if(route.indexOf('now managed in Platform Settings') < 0)
  fail('the retired route no longer explains where the setting moved to');
if(route.indexOf('saveNotificationConfig') >= 0 || route.indexOf('requireAdminRole') >= 0)
  fail('the admin route can write again');

// ---------- the Admin page must be READ-ONLY, and still useful ----------
if(page.indexOf('use client') >= 0)
  fail('the read-only view became a client component — it has no interactivity to justify one');
if(/onChange|onClick|<input|<button/.test(page))
  fail('the Admin page has editable controls again');
if(page.indexOf('Platform Settings') < 0)
  fail('the read-only view no longer says where the settings are managed');
if(page.indexOf('NOTIFICATION_TYPES.map') < 0)
  fail('the read-only view no longer lists the notification types — the operator fallback would show nothing');

// ---------- behaviour and permissions untouched ----------
if(svc.indexOf('if (!isKnownNotificationType(typeKey)) return false;') < 0)
  fail('isNotificationEnabled changed — this release must not alter notification behaviour');
if(svc.indexOf('for (const t of NOTIFICATION_TYPES) out[t.key] = effectiveType(t.key, stored);') < 0)
  fail('the effective-config merge changed');
" || exit 1

# The Platform owner surface and the notification consumers must not have moved.
OWNER_TOUCHED=$(git diff --name-only "${BASE_COMMIT}"..HEAD -- app/platform/dashboard/settings/notifications app/api/platform/notification-settings components/platform/NotificationSettingsWorkspace.tsx services/notifications/notificationCatalog.ts services/platformUsers/platformPermissions.ts)
if [ -n "$OWNER_TOUCHED" ]; then
  echo "ERROR: the owning surface, the catalogue or the permission matrix changed:"
  echo "$OWNER_TOUCHED" | sed 's/^/         /'
  echo "       SC-047 removes the DUPLICATE; the owner keeps working exactly as it did."
  exit 1
fi
echo "      confirmed: one writer, it still merges, admin path retired, owner untouched."
...
npx tsx scripts/sc001_cscs_tests.ts > /tmp/sc047_cscs.log 2>&1
if [ $? -ne 0 ]; then
  echo "ERROR: the CSCS verification suite failed."
  tail -15 /tmp/sc047_cscs.log | sed 's/^/         /'
  exit 1
fi
echo "      $(tail -2 /tmp/sc047_cscs.log | tr -d '\n' | sed 's/.*== //')"

echo "[4/7] Building and packaging..."
npx prisma generate > /dev/null 2>&1 || { echo "ERROR: prisma generate failed"; exit 1; }
npm run build > /tmp/sc047_build.log 2>&1
if [ $? -ne 0 ]; then
  echo "ERROR: build failed. Last lines:"
  tail -25 /tmp/sc047_build.log | sed 's/^/         /'
  exit 1
fi
grep -qE "Compiled successfully" /tmp/sc047_build.log || { echo "ERROR: build did not report success"; exit 1; }
...
if [ "$RC" = "200" ] && [ "$ROUTES_OK" = "1" ]; then
  echo "== SC-047 DEPLOYED =="
  echo "   Platform Settings owns notification settings; the Admin Centre view"
  echo "   is read-only. One write path to the singleton row."
else
  echo "== SC-047 DEPLOYED BUT NOT HEALTHY — INVESTIGATE =="
  exit 1
fi
