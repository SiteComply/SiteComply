#!/usr/bin/env bash
# SC-029 — production CODE deploy: SC-022 Save Access.
#
# Permission edits on the Access workspace are staged and committed by SAVE
# ACCESS instead of PATCHing on every checkbox. The rules are untouched: same
# endpoint, same per-module payload, same server-side narrowing, same RBAC,
# templates and site assignment logic — only WHEN the request is sent.
#
...
echo "== SC-029 — SC-022 SAVE ACCESS =="
...
OUTSIDE=$(git diff --name-only "${BASE_COMMIT}"..HEAD \
  | grep -v '^components/platform/SiteAccessManager.tsx$' \
  | grep -v '^scripts/' || true)
if [ -n "$OUTSIDE" ]; then
  echo "ERROR: SC-029 is an interaction change to one component, but these also changed:"
  echo "$OUTSIDE" | sed 's/^/         /'
  exit 1
fi
...
// ---------- SC-022 Save Access ----------
// NOTE: no double quotes in comments inside this node -e block — an unescaped
// one closes the shell string and silently truncates the program.
const am=strip('components/platform/SiteAccessManager.tsx');
if(am.indexOf('saveAccess') < 0) fail('the Save access handler is gone');
if(am.indexOf('Save access') < 0) fail('the Save access button label is gone');
if(am.indexOf('Discard changes') < 0) fail('the Discard control is gone');
if(!/sticky top-0/.test(am))
  fail('the Save access bar is no longer sticky — it would scroll away on a long list of people');
// THE DEFECT THIS FIXES. toggleVerb must STAGE, never send. If it calls send()
// again, every tick is an immediate permanent permission change and the Save
// button becomes decoration.
const tv=am.slice(am.indexOf('function toggleVerb'), am.indexOf('async function saveAccess'));
if(tv.length < 50) fail('toggleVerb is missing');
if(/\bsend\(/.test(tv))
  fail('toggleVerb sends immediately again — ticking a box would commit without Save');
if(tv.indexOf('setDraft') < 0) fail('toggleVerb no longer stages into the draft');
// The bar must only appear when something is pending.
if(!/pendingCount > 0/.test(am))
...
console.log('      confirmed: toggling stages instead of committing, Save access');
console.log('                 is present and sticky, the bar is conditional, and');
console.log('                 row actions cannot discard a pending draft.');
...
if [ "$CODE" = "200" ] && [ -z "$SMOKE_FAIL" ]; then
  echo "== SC-029 DEPLOYED =="
  echo "   Access rules, RBAC and templates UNCHANGED — only when the save happens."
else
