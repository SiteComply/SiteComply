#!/usr/bin/env bash
# COMPANY INDUCTION MODULES — the move into the Induction Videos area.
#
# NO MIGRATION. This is an information-architecture change: a page moved, its API
# moved with it, and a new view-permission was added. The schema is untouched, so
# the script asserts that rather than checking for a column.
#
# NOTHING AN OPERATIVE SEES CHANGES. No content, no resolution rule, no
# narration: a screen moved and a view-permission was added. What CAN go wrong is
# access - a page inheriting the wrong gate on its way into a new folder - so
# that is what the asserts are about, in both directions:
#
#   too wide    the Induction Videos area admits PROJECT_MANAGER and
#               PRINCIPAL_CONTRACTOR; company policy administration must not
#               follow the page into their reach.
#   too narrow  a PROJECT_MANAGER could read this screen through Settings, and
#               must still be able to.
#
# The governance asserts are kept from the B+C deploy verbatim, because "we only
# moved a page" is exactly the change under which nobody re-checks them.
set -uo pipefail
...
ZIP=/tmp/inductionmodules_ia_deploy.zip
...
echo "  ok   schema untouched, no migration to apply"
...
npx next build >/tmp/inductionmodules_ia_build.log 2>&1 || { tail -30 /tmp/inductionmodules_ia_build.log; fail "build failed"; }
...
for t in "Company induction content" "Leave it out of this project" \
         ...
         "Company modules" "Induction video areas" "Now managed with Induction videos"; do
...
for path in / /check-in /worker/permits /platform/dashboard/sites \
            ...
            /platform/dashboard/induction-videos/modules ; do
