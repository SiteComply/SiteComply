#!/usr/bin/env bash
# SC-033 — production CODE deploy: SC-001 CSCS Smart Check readiness.
#
...
APP=sitecomply-web
PG=sitecomply-pg
PRECHECK_RULE=tmp-sc033-precheck
SCM="https://${APP}.scm.azurewebsites.net"
BASE="https://${APP}.azurewebsites.net"
HEALTH="${BASE}/api/health"
ZIP=/tmp/sc033_deploy.zip
...
echo "== SC-033 — CSCS SMART CHECK READINESS =="
...
trap - EXIT
if [ "$PRESENT" != "2" ]; then
  echo "ERROR: production has ${PRESENT}/2 of the new CSCS tables."
  echo "       This build reads CscsConfig on EVERY card verification, which is"
  echo "       on the worker onboarding path — an un-migrated deploy would break"
  echo "       profile save for any worker entering a card number."
  echo "       RUN scripts/sc033_migrate.sh FIRST."
  exit 1
fi
...
OUTSIDE=$(git diff --name-only "${BASE_COMMIT}"..HEAD \
  | grep -v '^services/cscs/' \
  | grep -v '^components/admin/CscsProviderSettings.tsx$' \
  | grep -v '^app/admin/(dashboard)/settings/integrations/page.tsx$' \
  | grep -v '^app/api/admin/settings/cscs/' \
  | grep -v '^app/api/worker/profile/route.ts$' \
  | grep -v '^prisma/' \
  | grep -v '^scripts/' || true)
if [ -n "$OUTSIDE" ]; then
  echo "ERROR: SC-033 should touch only the CSCS path, but these also changed:"
  echo "$OUTSIDE" | sed 's/^/         /'
  exit 1
fi
...
// ---------- SC-033 CSCS Smart Check readiness ----------
...
// executes. Substring tests over regex for the same reason.
const csMap=strip('services/cscs/smartCheckMapper.ts');
const csProv=strip('services/cscs/smartCheckProvider.ts');
const csCfg=strip('services/cscs/cscsConfigService.ts');
const csVer=strip('services/cscs/cscsVerificationService.ts');
const csIdx=strip('services/cscs/index.ts');
const csUi=strip('components/admin/CscsProviderSettings.tsx');

// ---- the mapper must FAIL CLOSED ----
// If an unrecognised status ever maps to VALID, a payload nobody understood
// becomes a verified competency record. This is the assertion that matters
// most in the whole file.
if(csMap.indexOf('export function mapStatus') < 0) fail('the status mapper is gone');
if(csMap.indexOf('return ERROR_SENTINEL') >= 0) fail('unreachable');
const mapStatusBody=csMap.slice(csMap.indexOf('export function mapStatus'), csMap.indexOf('export function mapQualifications'));
// COUNT, not presence. mapStatus contains two ERROR returns — the guard clause
// for an empty status and the final fallback for an unrecognised one. A
// presence check passes when either is flipped to VALID, which was tried and
// slipped through; requiring both means flipping either one fails here.
const errReturn = 'return ' + String.fromCharCode(39) + 'ERROR' + String.fromCharCode(39);
const errCount = mapStatusBody.split(errReturn).length - 1;
if(errCount < 2)
  fail('mapStatus has only ' + errCount + ' ERROR return(s) — it must fail closed on BOTH an empty and an unrecognised status, or an unknown payload could read as valid');
// verified must never be read from the payload.
if(csMap.indexOf('verified: status ===') < 0)
  fail('verified is no longer derived from status alone — a payload could assert its own validity');
// The self-contradiction downgrade.
if(csMap.indexOf('status = ' + String.fromCharCode(39) + 'EXPIRED' + String.fromCharCode(39)) < 0)
  fail('a VALID card with a past expiry is no longer downgraded');

// ---- the provider must refuse rather than fabricate ----
if(csProv.indexOf('assertConfigured') < 0)
  fail('the Smart Check provider no longer refuses to run unconfigured');
if(csProv.indexOf('mapSmartCheckResponse') < 0)
  fail('the provider no longer uses the mapping layer');
// The blocked surface must stay isolated, so onboarding is one edit.
if(csProv.indexOf('REQUEST_SHAPE') < 0)
  fail('the request contract is no longer isolated — partner onboarding would become a hunt');

// ---- configuration must be real, not decorative ----
if(csIdx.indexOf('resolveCscsProvider') < 0)
  fail('the provider is no longer resolved from the runtime config');
if(csVer.indexOf('resolveCscsProvider') < 0)
  fail('the verification service ignores the configured provider');
// Selecting a provider that cannot run is the unenforced-setting defect.
if(csCfg.indexOf('needs a partner API URL and key') < 0)
  fail('Smart Check can be selected without credentials — a screen claiming verification is live while every check fails');
// The key must never leave the server.
if(csCfg.indexOf('encryptSecret') < 0) fail('the API key is no longer encrypted at rest');
if(csUi.indexOf('apiKeySet') < 0) fail('the settings screen may be returning the key itself');

// ---- audit ----
if(csVer.indexOf('cscsVerificationLog.create') < 0)
  fail('verification attempts are no longer audited');
if(csVer.indexOf('maskCardNumber') < 0)
  fail('card numbers are no longer masked in the audit trail');

// ---- verification must never block a worker ----
if(csVer.indexOf('catch') < 0)
  fail('the verification service can now throw — a provider failure would block a worker reaching site');

console.log('      confirmed: the mapper fails closed and derives verified from');
console.log('                 status alone, the provider refuses to run without');
console.log('                 credentials, the config is real and encrypted, every');
console.log('                 attempt is audited with a masked number, and a');
console.log('                 failure can never block a worker.');
...
for path in \
  ...
  # pattern. SC-033 printed "routes: all reachable" while every route was
...
if [ "$CODE" = "200" ] && [ -z "$SMOKE_FAIL" ]; then
  echo "== SC-033 DEPLOYED =="
  echo "   Verification still runs on the mock. Nothing changes until credentials"
  echo "   are entered and Smart Check is selected in Admin -> Integrations."
else
