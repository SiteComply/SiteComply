#!/usr/bin/env bash
# CPP PDF PAGINATION — the first page earns its place.
#
# The masthead had a sheet to itself and the contents another. On a small
# project that meant two sparse pages before the plan had started, and the
# opening read as unfinished.
#
#   - The CONTENTS now begins on page 1, directly under document control.
#   - The PLAN follows the contents directly. Removing only the contents break
#     moved the gap rather than closing it: the contents spilled a few rows onto
#     sheet two and the body break left the rest of that sheet blank.
#   - The PART HEADING is bound to its first section in a wrap={false} pair, so
#     it is never stranded at the foot of a sheet. minPresenceAhead was tried
#     first and did NOT hold - an 8-section plan still ended sheet one with the
#     heading alone.
#
# Measured, not assumed: page one goes from 22 lines to 41-57 across project
# sizes from 1 to 26 sections, and page counts fall (4 sections 4 -> 3,
# 20 sections 7 -> 6).
#
# Sections, the drawings block and the approval are all still unsplittable.
#
# NO MIGRATION.
#
set -uo pipefail
...
ZIP=/tmp/cpppagination_deploy.zip
...
npx next build >/tmp/cpppagination_build.log 2>&1 || {
  tail -30 /tmp/cpppagination_build.log; fail "build failed";
}
