#!/usr/bin/env bash
# OTP verification fix — production CODE deploy (commit eec0eff on top of SC-003).
# No database migration: this change is code-only (a new signed cookie + client
# error handling). Same proven flow as scripts/sc003_deploy.sh.
set -uo pipefail
...
echo "== OTP FIX CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming the OTP fix IS present in this artifact..."
grep -q 'sc_otp_mobile' lib/session.ts \
  && grep -q "getWorkerOtpMobile" app/api/worker/otp/verify/route.ts \
  && grep -q 'mobileError' app/check-in/page.tsx \
  && echo "      confirmed: OTP mobile-persistence + split errors present." \
  || { echo "ERROR: OTP fix not found in tree — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== OTP FIX DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
