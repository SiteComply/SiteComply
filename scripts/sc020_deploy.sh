#!/usr/bin/env bash
# SC-020 production CODE deploy (Mandatory Action Assignment). The additive
...
echo "== SC-020 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-020 code IS present in this artifact..."
[ -f services/compliance/occurrenceGenerator.ts ] \
  && [ -f services/compliance/scheduleService.ts ] \
  && [ -f services/compliance/occurrenceService.ts ] \
  && [ -f components/platform/ComplianceCalendar.tsx ] \
  && [ -f app/platform/dashboard/compliance-calendar/page.tsx ] \
  && grep -q 'ComplianceOccurrence' prisma/schema.prisma \
  && grep -q 'Daily Safe Start' services/audits/auditTemplateSeed.ts \
  && echo "      confirmed: generator + services + calendar + activity types present." \
  || { echo "ERROR: SC-020 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-020 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
