#!/usr/bin/env bash
# SC-009 production CODE deploy (Digital Permit to Work). The additive migration
# 20260728170000_add_permits + catalogue seed must already be applied (run
# scripts/sc009_migrate.sh first). Same proven flow as SC-005..008.
set -uo pipefail
...
echo "== SC-009 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-009 code IS present in this artifact..."
[ -f services/permits/permitService.ts ] \
  && [ -f services/permits/permitCatalogSeed.ts ] \
  && [ -f app/api/worker/permits/route.ts ] \
  && [ -f app/platform/dashboard/permits/page.tsx ] \
  && [ -f components/permits/PermitRequestForm.tsx ] \
  && grep -q "permits" services/platformUsers/platformPermissions.ts \
  && echo "      confirmed: permit services + APIs + worker/platform UI present." \
  || { echo "ERROR: SC-009 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-009 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
