#!/usr/bin/env bash
# SC-013 production CODE deploy (Audit Creation & Templates). The additive
# migration 20260729120000_add_audit_templates + template seed must already be
# applied (run scripts/sc013_migrate.sh first). Same proven flow as SC-005..012.
set -uo pipefail
...
echo "== SC-013 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-013 code IS present in this artifact..."
[ -f services/audits/auditTemplateService.ts ] \
  && [ -f services/audits/auditTemplateSeed.ts ] \
  && [ -f app/platform/dashboard/audits/templates/page.tsx ] \
  && [ -f app/api/platform/audit-templates/route.ts ] \
  && grep -q 'canManageAuditTemplates' services/platformUsers/platformPermissions.ts \
  && echo "      confirmed: audit template service + library UI + RBAC present." \
  || { echo "ERROR: SC-013 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-013 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
