#!/usr/bin/env bash
# SC-007 production CODE deploy (GPS Site Check-In Validation). The additive
# migration 20260728140000_add_gps_checkin must already be applied (run
# scripts/sc007_migrate.sh first). Same proven flow as the SC-005/SC-006 deploys.
set -uo pipefail
...
echo "== SC-007 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-007 code IS present in this artifact..."
[ -f services/geo/geoValidationService.ts ] \
  && [ -f services/geo/geoConfigService.ts ] \
  && [ -f app/api/worker/location-check/route.ts ] \
  && [ -f app/api/platform/sites/\[id\]/gps/route.ts ] \
  && [ -f components/checkin/LocationCheck.tsx ] \
  && [ -f components/platform/GpsCheckInConfig.tsx ] \
  && grep -q 'evaluateCheckInGate' services/submissions/submissionService.ts \
  && echo "      confirmed: geo services + location-check API + GPS config UI present." \
  || { echo "ERROR: SC-007 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-007 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
