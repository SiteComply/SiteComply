#!/usr/bin/env bash
# SC-043 — production CODE deploy: CSCS status banner fix (M-5).
#
# The banner and the dropdown description shared one binding, derived from
# FORM state, so an unsaved selection made the banner name a provider that was
# not running — in both directions, including claiming Smart Check was
# verifying while the mock was live.
#
# PRESENTATION ONLY. One line moves from `selected` (the dropdown) to `live`
# (the saved runtime config). Nothing about how settings are saved, stored or
# read at verification time changes.
#
# THE RISK IS FIXING IT THE WRONG WAY ROUND. Pointing the DESCRIPTION at the
# saved provider instead would make the select explain the wrong option, so
# [3/7] asserts both bindings independently: the banner must read the saved
# config, and the description must still follow the dropdown.
#
...
echo "== SC-043 — CSCS STATUS BANNER FIX (M-5) =="
...
if [ -n "$TOUCHED" ]; then
  ...
  echo "$TOUCHED" | sed 's/^/         /'
  echo "       SC-043 is a colour change. If the devCode suppression has been"
...
OUTSIDE=$(git diff --name-only "${BASE_COMMIT}"..HEAD \
  ...
  | grep -v '^scripts/' || true)
if [ -n "$OUTSIDE" ]; then
  echo "ERROR: SC-043 fixes ONE defect, but these also changed:"
  echo "$OUTSIDE" | sed 's/^/         /'
  exit 1
fi
...
const ui=strip('components/admin/CscsProviderSettings.tsx');

// ---------- the banner must read the SAVED config ----------
if(ui.indexOf('const live = config.providers.find((p) => p.id === config.activeProvider);') < 0)
  fail('the live binding is gone — the banner would name the dropdown selection again');
if(ui.indexOf('{live?.name ?? config.activeProvider}') < 0)
  fail('the banner no longer renders the live provider');
if(ui.indexOf('{selected?.name ?? config.activeProvider}') >= 0)
  fail('the banner reads the dropdown selection again — that is the defect itself');

// ---------- ...and the DESCRIPTION must still follow the dropdown ----------
// Fixing this the wrong way round would leave the select explaining an option
// the user is not looking at.
if(ui.indexOf('const selected = config.providers.find((p) => p.id === activeProvider);') < 0)
  fail('selected no longer follows the dropdown — the description under the select would explain the wrong option');
if(ui.indexOf('{selected.description}') < 0)
  fail('the provider description is gone from the select');

// ---------- presentation only ----------
if(ui.indexOf('Currently verifying with') < 0)
  fail('the status banner itself has been removed');
for(const banned of ['prisma.', 'encryptSecret', 'process.env']){
  if(ui.indexOf(banned) >= 0)
    fail('the component reaches for '+banned+' — this is a presentation change to a client component');
}
" || exit 1

# Settings storage and the verification path must not have been dragged in.
BACKEND_TOUCHED=$(git diff --name-only "${BASE_COMMIT}"..HEAD -- services/cscs app/api/admin/settings/cscs)
if [ -n "$BACKEND_TOUCHED" ]; then
  echo "ERROR: CSCS backend code changed:"
  echo "$BACKEND_TOUCHED" | sed 's/^/         /'
  echo "       SC-043 changes what the banner DISPLAYS, never what is stored or read."
  exit 1
fi
echo "      confirmed: banner reads saved config, description follows the dropdown, backend untouched."
...
npx tsx scripts/sc001_cscs_tests.ts > /tmp/sc043_cscs.log 2>&1
if [ $? -ne 0 ]; then
  echo "ERROR: the CSCS verification suite failed."
  tail -15 /tmp/sc043_cscs.log | sed 's/^/         /'
  exit 1
fi
echo "      $(tail -2 /tmp/sc043_cscs.log | tr -d '\n' | sed 's/.*== //')"

echo "[4/7] Building and packaging..."
npx prisma generate > /dev/null 2>&1 || { echo "ERROR: prisma generate failed"; exit 1; }
npm run build > /tmp/sc043_build.log 2>&1
if [ $? -ne 0 ]; then
  echo "ERROR: build failed. Last lines:"
  tail -25 /tmp/sc043_build.log | sed 's/^/         /'
  exit 1
fi
grep -qE "Compiled successfully" /tmp/sc043_build.log || { echo "ERROR: build did not report success"; exit 1; }
...
if [ "$RC" = "200" ] && [ "$ROUTES_OK" = "1" ]; then
  echo "== SC-043 DEPLOYED =="
  echo "   The CSCS banner now reports the provider actually in use."
  echo "   Presentation only. Nothing else changed."
else
  echo "== SC-043 DEPLOYED BUT NOT HEALTHY — INVESTIGATE =="
  exit 1
fi
