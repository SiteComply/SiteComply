#!/usr/bin/env bash
# SC-019 production CODE deploy (Mandatory Action Assignment). The additive
# migration 20260729120000_add_audit_templates + template seed must already be
# applied (run scripts/sc019_migrate.sh first). Same proven flow as SC-005..012.
set -uo pipefail
...
echo "== SC-019 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-019 code IS present in this artifact..."
[ -f services/sites/siteSetupService.ts ] \
  && [ -f services/sites/siteSetupConstants.ts ] \
  && [ -f components/platform/SiteSetupWizard.tsx ] \
  && [ -f "app/platform/dashboard/sites/[id]/setup/page.tsx" ] \
  && [ -f "app/api/platform/sites/[id]/setup/route.ts" ] \
  && grep -q 'CdmDutyHolders' prisma/schema.prisma \
  && echo "      confirmed: setup service + constants + wizard + page + API present." \
  || { echo "ERROR: SC-019 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-019 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
