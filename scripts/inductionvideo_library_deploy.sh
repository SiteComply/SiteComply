#!/usr/bin/env bash
# THE VIDEO LIBRARY, plus the "Videos" -> "Sites" rename.
#
# MIGRATIONS FIRST: ~/library_run.sh (four tables, two enums, four nullable scene
# columns).
#
# THE FRAMERATE CHANGES FOR EVERY RENDER. 10 -> 25, because the join is a stream
# copy and a static scene cannot run at a different rate from a piece of film.
# Generated scenes therefore cost more CPU on a shared instance; rendering is a
# queued job whose page refreshes itself, so this is spend rather than breakage.
#
# WHAT MUST BE TRUE:
#   ONE SPEC        the normaliser imports videoFormat.ts rather than repeating it.
#                   Two copies would agree until somebody changed one, and the
#                   symptom is a concatenated file that will not play.
#   NOTHING HALF-   an un-transcoded revision must be invisible to a site, and
#   BAKED           captions must gate issuing.
#   NO DUPLICATION  footage that covers a module displaces it.
#   FROZEN RECORD   the scene carries the segment path, so a re-issue cannot change
#                   an already-published induction.
set -uo pipefail
...
ZIP=/tmp/inductionvideo_library_deploy.zip
...
echo "[3/7] Asserting the migrations landed, and the guards..."
...
TBL=$(PGOPTIONS='-c default_transaction_read_only=on' psql "$DB" -X -tA -c "SELECT count(*) FROM information_schema.tables WHERE table_name IN ('LibraryAsset','LibraryAssetRevision','LibraryAssetEvent','SiteLibraryAsset','LibraryNormaliseJob')" 2>/dev/null)
[ -n "$TBL" ] || fail "could not check the production schema - refusing to deploy blind"
[ "$TBL" = "5" ] || fail "production has $TBL of the 5 library tables - run ~/library_run.sh first"
COL=$(PGOPTIONS='-c default_transaction_read_only=on' psql "$DB" -X -tA -c "SELECT count(*) FROM information_schema.columns WHERE table_name='InductionVideoScene' AND column_name LIKE 'library%'" 2>/dev/null)
[ "$COL" = "4" ] || fail "InductionVideoScene has $COL of the 4 library columns"
echo "  ok   production has all five library tables and the four scene columns"
...
for L in "'Sites'" "'Company modules'" "'Library'"; do
...
LSVC=services/inductionVideo/libraryAssetService.ts
LACT=services/inductionVideo/libraryActions.ts
LSEC=components/inductionVideo/LibrarySection.tsx
FMT=services/inductionVideo/videoFormat.ts
for f in "$NORM" "$LSVC" "$LACT" "$LSEC" services/inductionVideo/libraryRows.ts \
         components/inductionVideo/SiteLibraryPanel.tsx \
         app/api/platform/induction-library/route.ts \
         app/api/admin/induction-library/route.ts \
         app/platform/dashboard/induction-videos/library/page.tsx \
         "app/admin/(dashboard)/induction-videos/library/page.tsx"; do
  test -f "$f" || fail "missing: $f"
done
# ONE framerate, high enough for footage.
grep -q "fps: 25," "$FMT" \
  || fail "the pipeline is not at 25 fps - footage would judder, and a stream copy cannot mix rates"
grep -q "from '@/services/inductionVideo/videoFormat'" "$NORM" \
  || fail "the normaliser does not import the output spec - two copies would drift"
grep -q "force_original_aspect_ratio=decrease" "$NORM" \
  || fail "the normaliser crops instead of padding"
grep -q "anullsrc=channel_layout=stereo:sample_rate=44100" "$NORM" \
  || fail "silent footage would have no audio stream and could not be concatenated"
grep -q "'-ar', '44100'" "$NORM" || fail "the normaliser's audio spec does not match the renderer's"
# Nothing half-baked reaches a site.
grep -q "if (!issued.normalisedBlobPath || !issued.captionsBlobPath) continue;" "$LSVC" \
  || fail "an un-transcoded revision could reach a site and break its render"
grep -q "if (!readiness.ready) {" "$LSVC" \
  || fail "a revision could be issued without captions or footage"
grep -q "missing.push('a caption file')" "$LSVC" \
  || fail "captions are no longer required to issue"
grep -q "if (!actor.canIssue) {" "$LSVC" || fail "issuing is not gated"
# No duplication, and the record stays frozen.
grep -q "if (replacedModuleIds.has(m.moduleId)) {" services/inductionVideo/sceneRules.ts \
  || fail "a written module would play alongside the footage that covers it"
grep -qF "s.libraryRevisionId ?? ''," services/inductionVideo/sceneRules.ts \
  || fail "the manifest hash ignores the library revision - a re-issue would leave stale renders looking current"
grep -q "scene.libraryBlobPath" services/inductionVideo/renderService.ts \
  || fail "the renderer does not read the segment path off the scene"
grep -q "if (scene.segment) {" services/inductionVideo/ffmpegRenderer.ts \
  || fail "footage would be re-encoded instead of concatenated"
grep -q "if (startMs >= limitMs) continue;" services/inductionVideo/captions.ts \
  || fail "a caption could run past its segment and caption the wrong picture"
# Uploads bypass the application, write-only, drafts only.
grep -q "BlobSASPermissions.parse('cw')" services/inductionVideo/mediaStorage.ts \
  || fail "the upload SAS is not write-only"
grep -q "rev.status !== 'DRAFT'" "$LACT" \
  || fail "an upload URL could be issued for an ISSUED revision"
grep -q "return \`library/\${assetId}/\${revisionId}\`" services/inductionVideo/mediaStorage.ts \
  || fail "library media is not under its own prefix - the retention sweep would eat the masters"
# One system, two front doors.
for R in app/api/platform/induction-library/route.ts app/api/admin/induction-library/route.ts; do
  grep -q "handleLibraryAction(" "$R" || fail "$R does not use the shared dispatcher"
  grep -qE "createLibraryAsset\(|issueRevision\(" "$R" \
    && fail "$R implements an action itself"
done
# Three areas, and the rename.
grep -q "key: 'sites'," services/inductionVideo/inductionVideoAreas.ts \
  || fail "the Sites area is missing or still called videos"
grep -q "label: 'Sites'," services/inductionVideo/inductionVideoAreas.ts \
  || fail "the tab is not labelled Sites"
grep -q "key: 'library'," services/inductionVideo/inductionVideoAreas.ts \
  || fail "the Library area is not registered"
grep -q "label: 'Videos'," services/inductionVideo/inductionVideoAreas.ts \
  && fail "the old Videos label is back"
echo "  ok   library asserts pass (25)"
...
for PART in "s.moduleRevisionId ?? ''," "s.libraryRevisionId ?? ''," ; do
  grep -qF "$PART" "$RULES" \
    || fail "the manifest hash no longer includes ${PART}"
done
...
npx next build >/tmp/inductionvideo_library_build.log 2>&1 || { tail -30 /tmp/inductionvideo_library_build.log; fail "build failed"; }
...
for t in "Company induction content" "Leave it out of this project" \
         ...
         "Delete version" "Yes, delete it" \
         "Company video library" "Upload captions" "Company videos"; do
...
test -f ".next/server/app/admin/(dashboard)/induction-videos/library/page.js" \
  || fail "the Admin library page did not build"
test -f ".next/server/app/platform/dashboard/induction-videos/library/page.js" \
  || fail "the Platform library page did not build"
test -f ".next/server/app/api/admin/induction-library/route.js" \
  || fail "the Admin library API did not build"
test -f ".next/server/app/api/platform/induction-library/route.js" \
  || fail "the Platform library API did not build"
...
for path in / /check-in /worker/permits /platform/dashboard/sites \
            ...
            /admin/induction-videos/projects/x \
            /admin/induction-videos/library \
            /platform/dashboard/induction-videos/library ; do
