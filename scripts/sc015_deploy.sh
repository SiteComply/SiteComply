#!/usr/bin/env bash
# SC-015 production CODE deploy (Mandatory Action Assignment). The additive
# migration 20260729120000_add_audit_templates + template seed must already be
# applied (run scripts/sc015_migrate.sh first). Same proven flow as SC-005..012.
set -uo pipefail
...
echo "== SC-015 CODE DEPLOY =="
echo "on commit: $(git rev-parse --short HEAD) ($(git rev-parse --abbrev-ref HEAD))"

echo "[1/8] Current prod build id:"
OLD_BUILD=$(kudu_buildid); echo "      OLD_BUILD=${OLD_BUILD:-<unknown>}"

echo "[2/8] Confirming SC-015 code IS present in this artifact..."
[ -f services/actions/actionAssigneeService.ts ] \
  && [ -f "app/api/platform/sites/[id]/assignable-people/route.ts" ] \
  && grep -q 'assignedWorkerId' prisma/schema.prisma \
  && grep -q "mode: 'create' | 'edit'" services/actions/actionService.ts \
  && echo "      confirmed: assignee service + endpoint + mandatory validation present." \
  || { echo "ERROR: SC-015 code missing — aborting"; exit 1; }
...
[ "$CODE" = "200" ] && echo "== SC-015 CODE DEPLOY COMPLETE ==" || echo "== HEALTH NOT 200 — investigate =="
