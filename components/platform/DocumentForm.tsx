    ...
    >
      ...
      {mode === 'upload' ? (
        <div className="space-y-1.5">
          ...
          {errors.file ? (
            <p className="text-sm font-medium text-danger-600">{errors.file}</p>
          ) : (
            <p className="text-sm text-ink-subtle">{ACCEPTED_DOCUMENTS_HINT}</p>
          )}
        </div>
      ) : (
        existingFile && (
          <div className="rounded-xl border border-line bg-surface-sunken px-4 py-3 text-sm">
            ...
            <p className="mt-1 text-xs text-ink-subtle">
              The file cannot be replaced in this phase — upload a new document
              instead.
            </p>
          </div>
        )
