...
- Captured at: commit `44841c5` on branch `feature/ux-refresh`
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Dashboard (not a priority) | 393 | 5 | **7** | 5 | 0 | 7 | 0 | 0 |
| Sites register | 196 | 5 | **7** | 4 | 0 | 3 | 6 | 0 |
...
```
12:  assertModuleView,
63:  assertModuleView(viewer, 'dashboard');
70:  const canActions = permits(viewer.role, 'actions', 'view');
71:  const canDocs = permits(viewer.role, 'documents', 'view');
72:  const canAudits = permits(viewer.role, 'audits', 'view');
73:  const canCheckins = permits(viewer.role, 'checkins', 'view');
74:  const canSites = permits(viewer.role, 'sites', 'view');
```
### Sites register
```
15:  assertModuleView,
43:  assertModuleView(viewer, 'sites');
45:  const canExport = permits(viewer.role, 'sites', 'export');
```
### Site → Overview
```
7:  canCloseProject,
...
```
7:  assertModuleView,
52:  assertModuleView(viewer, 'actions');
79:  const canCreate = permits(viewer.role, 'actions', 'create');
```
### Action detail
```
8:  assertModuleView,
...
```
7:  assertModuleView,
39:  assertModuleView(viewer, 'audits');
56:  const canCreate = permits(viewer.role, 'audits', 'create');
```
### Audit detail
```
8:  assertModuleView,
...
```
6:  assertModuleView,
34:  assertModuleView(viewer, 'permits');
```
### Permit detail
```
7:  assertModuleView,
...
```
7:  assertModuleView,
46:  assertModuleView(viewer, 'documents');
66:  const canCreate = permits(viewer.role, 'documents', 'create');
68:  const canDownload = permits(viewer.role, 'documents', 'export');
```
### Check-ins register
```
9:  assertModuleView,
37:  assertModuleView(viewer, 'checkins');
39:  const canExport = permits(viewer.role, 'checkins', 'export');
```
### Worker detail
```
10:  assertModuleView,
...
```
7:  assertModuleView,
22:  assertModuleView(viewer, 'reports');
```
### Report → Org overview
```
16:  assertModuleView,
...
```
6:  assertModuleView,
28:  assertModuleView(viewer, 'sites');
```
...
PlatformShell sidebar      : md:w-52
  sidebar rendered as card : 1          <- nav competing with content
...
Nav items (flat)           : 11
Nav grouping constructs    : 0          <- 0 = ungrouped, addressed in Phase 1
