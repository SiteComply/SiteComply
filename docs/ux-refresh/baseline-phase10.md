...
- Captured at: commit `c1e6715` on branch `feature/ux-refresh`
...
| Sites register | 179 | 4 | **7** | 3 | 0 | 3 | 4 | 0 |
| Site → Overview | 174 | 1 | **17** | 2 | 0 | 12 | 0 | 0 |
| Site → Worker Experience  ** BIGGEST ISSUE ** | 386 | 0 | **35** | 0 | 0 | 20 | 0 | 0 |
| Site → Compliance | 421 | 2 | **20** | 5 | 0 | 15 | 3 | 0 |
| Site → Workers | 401 | 1 | **35** | 7 | 0 | 8 | 3 | 0 |
| Site → Documents | 72 | 1 | **7** | 0 | 0 | 3 | 0 | 0 |
| Site → Access | 118 | 2 | **18** | 4 | 0 | 5 | 2 | 0 |
| Project Setup wizard | 152 | 0 | **16** | 0 | 0 | 3 | 0 | 0 |
| CPP draft | 255 | 4 | **6** | 12 | 0 | 0 | 1 | 7 |
...
| Permit detail | 223 | 5 | **11** | 7 | 0 | 2 | 0 | 0 |
| Documents register | 276 | 5 | **10** | 6 | 0 | 4 | 1 | 0 |
| Check-ins register | 259 | 3 | **7** | 5 | 0 | 3 | 2 | 0 |
| Worker detail | 347 | 6 | **8** | 15 | 0 | 3 | 3 | 0 |
| Notifications | 51 | 1 | **9** | 1 | 0 | 0 | 0 | 0 |
| Reports hub | 92 | 2 | **4** | 2 | 0 | 2 | 0 | 0 |
...
| Settings hub | 54 | 0 | **2** | 0 | 0 | 4 | 0 | 0 |
| Settings → Config templates | 28 | 0 | **2** | 0 | 0 | 1 | 0 | 0 |
| Settings → Permission templates | 92 | 0 | **17** | 1 | 0 | 5 | 0 | 0 |

**Totals across in-scope screens — cards 85, incl. nested 451, permission gates 158.**
...
```
17:  assertModuleView,
45:  assertModuleView(viewer, 'sites');
47:  const canExport = permits(viewer.role, 'sites', 'export');
```
### Site → Overview
```
7:  canCloseProject,
...
21:  assertModuleView,
25:  canEditSite,
76:  assertModuleView(viewer, 'sites');
78:  const canViewBulletins = permits(viewer.role, 'bulletins', 'view');
79:  const canPublishBulletins = permits(viewer.role, 'bulletins', 'create');
80:  const canManageBulletins = permits(viewer.role, 'bulletins', 'edit');
81:  const canConfigureDashboard = permits(viewer.role, 'sites', 'edit');
82:  const canEdit = canEditSite(viewer.role);
```
### Site → Compliance
```
21:  assertModuleView,
66:  assertModuleView(viewer, 'sites');
68:  const canViewCheckins = permits(viewer.role, 'checkins', 'view');
69:  const canViewAudits = permits(viewer.role, 'audits', 'view');
70:  const canViewActions = permits(viewer.role, 'actions', 'view');
146:  const canEditServices = permits(viewer.role, 'sites', 'edit');
```
### Site → Workers
```
15:  assertModuleView,
45:  assertModuleView(viewer, 'sites');
46:  if (!permits(viewer.role, 'checkins', 'view')) {
```
### Site → Documents
```
12:  assertModuleView,
...
```
8:  assertModuleView,
27:  assertModuleView(viewer, 'permits');
```
### Documents register
```
12:  assertModuleView,
...
```
17:  assertModuleView,
45:  assertModuleView(viewer, 'checkins');
47:  const canExport = permits(viewer.role, 'checkins', 'export');
```
### Worker detail
```
10:  assertModuleView,
