...
/**
 * SC-024 Phase 3 note: CLOSE_OUT_PACK is a valid AiSummaryTarget (it is written
 * to the AiSummary audit log and counts against the same usage caps) but it is
 * NOT a summary target. Its output shape, prompt and output guard differ, so it
 * is generated by services/closeOut/closeOutAi.ts rather than generateSummary.
 * Omitting it here is what keeps `generateSummary` unable to produce it.
 */
export const SUMMARY_TARGETS: Record<
  Exclude<AiSummaryTarget, 'CLOSE_OUT_PACK'>,
  SummaryTargetDef
> = {
...
/**
 * Look up a summary target, or null when the value is not one.
 *
 * CLOSE_OUT_PACK is the case that matters: it is a valid AiSummaryTarget for
 * logging but has no summary definition, so callers must handle its absence
 * rather than index into `undefined`.
 */
export function getSummaryTarget(
  targetType: AiSummaryTarget,
): SummaryTargetDef | null {
  return (
    (SUMMARY_TARGETS as Partial<Record<AiSummaryTarget, SummaryTargetDef>>)[
      targetType
    ] ?? null
  );
}
