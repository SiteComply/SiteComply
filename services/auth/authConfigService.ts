...
export const AUTH_LIMITS = {
  ...
  otpMaxAttempts: { default: 5, min: 3, max: 10, envKey: 'OTP_MAX_VERIFY_ATTEMPTS' },
  // Platform session lifetime — 15 minutes to 30 days.
  sessionTtlSeconds: { default: 60 * 60 * 8, min: 900, max: 60 * 60 * 24 * 30, envKey: 'PLATFORM_TTL_SECONDS' },
} as const;
...
export async function saveAuthConfig(
  ...
  const ttl = parseNumeric('otpTtlSeconds', input.otpTtlSeconds, 'OTP expiry (seconds)');
  const attempts = parseNumeric('otpMaxAttempts', input.otpMaxAttempts, 'Max OTP attempts');
  const session = parseNumeric('sessionTtlSeconds', input.sessionTtlSeconds, 'Session timeout (seconds)');
