...
const SYSTEM_PROMPT = [
  ...
  'check that verifies an operative understood THIS specific site induction. The goal',
  'is real understanding of safety-critical information — not recognition of',
  'trivia or the induction’s wording.',
  '',
  'GROUNDING (the correct answer):',
  '- Each question and its ONE correct answer must come from a fact EXPLICITLY',
  '  STATED in the briefing below. Quote or closely paraphrase that fact.',
  '- Do NOT rely on general construction, H&S, CSCS or regulatory knowledge, or',
  '  anything an operative would need from outside the briefing. If a fact is not in',
  '  the briefing, do not ask about it and do not assume it.',
  '- An operative who read the briefing must be able to answer with certainty.',
  '',
  'DISTRACTORS (the three wrong answers) — this is what makes a good question:',
  '- Each distractor must be the SAME KIND of thing as the correct answer and',
  '  comparable in length and form: locations vs locations, phone numbers vs phone',
  '  numbers, actions vs actions, PPE vs PPE. All four options must be plausibly',
  '  interchangeable so the answer cannot be spotted by shape.',
  '- Distractors must be realistic, plausible alternatives that are FALSE for THIS',
  '  site. It is fine to compose a believable wrong option (e.g. another sensible',
  '  muster point) even if it is not in the briefing — but it MUST be clearly',
  '  wrong for this site, and NEVER something the briefing states is true.',
  '- Do NOT build distractors by copying unrelated facts from other parts of the',
  '  briefing (a site name, a pledge and a phone number are not alternatives to a',
  '  location). That produces obviously-wrong options and is not allowed.',
  '- Never use "all/none of the above".',
  '',
  'WHAT TO ASK ABOUT — prioritise safety-critical understanding, in this order:',
  '  1. Emergency procedures — fire assembly point, first aider (who/where/number),',
  '     emergency number, nearest A&E.',
  '  2. Significant site hazards and the controls for them.',
  '  3. Mandatory PPE and site rules the operative must follow.',
  '  4. Permit-to-work, RAMS, stop-work authority, near-miss reporting.',
  'Prefer applied/scenario phrasing where the facts allow ("There is a fire — where',
  'do you go?", "Before starting hot works, what must you have?") over flat recall.',
  '',
  ...
  '- anything requiring a fact the briefing does not state (e.g. do not ask whether',
  '  an item is "mandatory" unless the briefing says so).',
  '',
  'Style:',
  ...
  '',
  'Quality over quantity: generate ONLY safety-relevant, well-grounded questions',
  'the briefing genuinely supports, up to the requested number. Returning fewer',
  'strong questions is REQUIRED — never pad with trivia or weak questions.',
].join('\n');
...
const META_PATTERNS = [
  ...
  /\bwhat does the (checklist|induction|briefing|list|site) (ask|say|state)\b/i,
  /\b(PPE_CONFIRM|YES_NO|ACKNOWLEDGEMENT)\b/,
  // v3: administrative trivia — not a safety-critical knowledge check.
  /\bwhich site is this\b/i,
  /\bwhat is the (site|job) (name|reference|number)\b/i,
  /\bwhich statement (does|do) the worker\b/i,
  /\bwhat does the worker confirm\b/i,
  // v3: positive "membership" phrasing tests recognition of the list, not
  // understanding (e.g. "which PPE item is listed…", "which question is included…").
  /\bwhich .*\b(is|are)\b.*\b(listed|included)\b/i,
  /\bwhich question\b/i,
];
